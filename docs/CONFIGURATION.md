...
The `certops` worker target runs nine independent sweeps. Each has an enable
...
| Diagnostic agent inactivity | `CERTOPS_SWEEP_DIAGNOSTIC_AGENT_INACTIVITY_ENABLED` | `CERTOPS_SWEEP_DIAGNOSTIC_AGENT_INACTIVITY_TIMEOUT_MS` | Retires a `diagnostic`-mode agent past its 24-hour inactivity TTL (ADR-0012 decision 7) |
| Agent recovery alerts | `CERTOPS_SWEEP_AGENT_RECOVERY_ALERTS_ENABLED` | `CERTOPS_SWEEP_AGENT_RECOVERY_ALERTS_TIMEOUT_MS` | Detects an agent's down->recovered transition and queues the corresponding alert |
| Trust-anchor reconciliation | `CERTOPS_SWEEP_TRUST_ANCHOR_RECONCILIATION_ENABLED` | `CERTOPS_SWEEP_TRUST_ANCHOR_RECONCILIATION_TIMEOUT_MS` | Reschedules or marks stale a `certops_trust_anchor_installations` row still pending past its `next_reconcile_at` (ADR-0012 decision 20b/20f/20h) |
