...
| `WEBHOOK_PROVIDER_HOSTS`                  | Legacy alias for `WEBHOOK_EXTRA_PROVIDER_HOSTS`, read only as a fallback when that variable is unset/empty. Neither variable replaces the built-in provider allowlist (Slack, Discord, Teams incl. legacy `office.com`/`office365.com`, PagerDuty, and Power Automate/Logic Apps `*.logic.azure.com` / `*.environment.api.powerplatform.com`), which is always allowed in addition to whatever this adds. Prefer `WEBHOOK_EXTRA_PROVIDER_HOSTS` for new entries. | `empty (built-in list still allowed)` | Webhook security     |
| `WEBHOOK_EXTRA_PROVIDER_HOSTS`            | Additional allowed hosts, unioned with the built-in provider allowlist above (comma-separated) | `empty`                               | Webhook security     |
...
| Variable              | Description                                                                     | Default value | Scope       |
| --------------------- | -------------------------------------------------------------------------------- | -------------- | ----------- |
| `NODE_USE_ENV_PROXY`   | Set to `1` to make Node's global `fetch`/undici honor `HTTP_PROXY`/`HTTPS_PROXY`/`NO_PROXY`. Requires Node **22.21.0+ or 24.5.0+** (not any `23.x` release, not `24.0.0`-`24.4.x`). On an unsupported version the API and worker log a non-fatal startup warning and `fetch`/undici simply ignore the proxy vars (`axios` keeps working regardless). Compose input: `TOKENTIMER_USE_ENV_PROXY`. | `unset (disabled)` | API, worker |
