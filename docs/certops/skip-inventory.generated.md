...
| `packages/agent/src/config/config.test.js` | 662 | sets 0600 permissions on the pin file on non-win32 platforms | it(..., { skip }) option-object | `no-host` |
| `packages/agent/src/config/config.test.js` | 759 | refuses to write the pin through a symlink | it(..., { skip }) option-object | `no-host` |
| `packages/agent/src/config/config.test.js` | 856 | sets 0600 permissions on the credential file on non-win32 platforms | it(..., { skip }) option-object | `no-host` |
| `packages/agent/src/config/config.test.js` | 916 | refuses a credential file whose ACL grants Everyone (win32) | it(..., { skip }) option-object | `no-host` |
| `packages/agent/src/config/config.test.js` | 931 | refuses a credential file owned by an untrusted principal (win32) | it(..., { skip }) option-object | `no-host` |
| `packages/agent/src/config/config.test.js` | 944 | refuses a credential file owned by an untrusted principal (win32) | dynamic (this.skip() / t.skip()) | `no-host` |
| `packages/agent/src/config/config.test.js` | 1258 | refuses a group/other-readable credentials file (POSIX) | it(..., { skip }) option-object | `no-host` |
| `packages/agent/src/config/config.test.js` | 1268 | refuses a credentials file whose ACL grants Everyone (win32) | it(..., { skip }) option-object | `no-host` |
| `packages/agent/src/config/config.test.js` | 1279 | accepts a credentials file restricted to the agent and SYSTEM (win32) | it(..., { skip }) option-object | `no-host` |
| `packages/agent/src/config/config.test.js` | 1355 | refuses a group/other-readable credentials file (POSIX) | it(..., { skip }) option-object | `no-host` |
| `packages/agent/src/config/config.test.js` | 1367 | refuses an ACL granting Everyone and never falls back to a skip (win32) | it(..., { skip }) option-object | `no-host` |
| `packages/agent/src/config/config.test.js` | 1380 | fails closed when the ACL cannot be inspected at all (win32) | it(..., { skip }) option-object | `no-host` |
