...
- The agent's local outbox now quarantines a transient (unclassified) delivery failure into a `dead-letter` subdirectory only after it has been retrying for 48 hours from creation, rather than after a small fixed attempt count (previously roughly 74-110 minutes); a permanent-error classification is still quarantined immediately. Both the main outbox and the dead-letter subdirectory are now retention-capped (5000 entries / 7 days for the main outbox, 5000 entries / 30 days for dead-letter) and pruned on a rate-limited cadence from the agent's poll loop.
