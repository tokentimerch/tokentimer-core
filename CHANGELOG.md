...
- **Workspace setting to always require approval before a certificate job runs.** Admins can turn on `certOpsRequireApprovalAlways` in a workspace's CertOps settings (`GET`/`PUT /api/v1/workspaces/:id/certops/settings`) so every new job - manual, bulk renew, scheduled renewal, and trust-anchor distribute/revoke - starts `pending_approval` regardless of what the caller passed for its own per-job approval flag. Off by default. The dashboard's manual job-creation modal shows and locks the approval checkbox as checked while the setting is on, and the workspace kill-switch panel gets a matching admin-only toggle. Turning it on or off is audited (`CERTOPS_WORKSPACE_APPROVAL_POLICY_ENABLED`/`_DISABLED`), same as the existing pause/resume kill switch. A request that sets both this and the pause flag together applies them as one transaction, so a failure partway through leaves neither setting changed.
