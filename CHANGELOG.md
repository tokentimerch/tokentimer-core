...
- **Workspace token transfer keeps endpoint monitors and managed certificates with the token.** `POST /api/v1/workspaces/:id/transfer-tokens` moves linked `domain_monitors`, managed-certificate inventory, and related CertOps rows in the same transaction. A fingerprint or job idempotency collision in the destination returns 409 `TRANSFER_CONFLICT` and leaves the source unchanged.
