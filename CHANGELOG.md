...
- **Machine trust-store distribution for CA trust anchors.** Operators can approve a CA certificate (root or intermediate) as a workspace trust anchor and distribute it to, or remove it from, a specific agent's machine trust store. Windows (`LocalMachine\Root` / `LocalMachine\CA`), Debian/Ubuntu (`update-ca-certificates`), and RHEL/Fedora (`update-ca-trust`) are supported; an agent advertises the `trust-anchor-deploy-v1` capability only on a platform where the required trust-store directory and update command actually resolve, and the control plane will not dispatch a trust job to, or let one be claimed by, an agent whose capability declaration is missing or stale.
  ...
  - Already-present material is detected against the real trust store rather than a filename convention, on every platform, so a CA an operator installed independently of TokenTimer is reported as already-present instead of being duplicated. The agent-local `allowedCommands` policy gate covers trust work on every platform too, including Windows under the `trust-store:certutil` profile name: a renew-only agent with no profiles configured refuses trust-anchor jobs rather than running them unconditionally.
  - New database migrations add the `certops_trust_anchors` and `certops_trust_anchor_installations` tables, plus `agent_id`/`last_job_id`/`transition_generation`/`last_attempt_at`/`last_error`/`next_reconcile_at` columns on the installations table, and the `certops_trust_reference_release_idempotency` ledger backing scoped reference-release idempotency. Requires PostgreSQL 15+ (see Configuration).
  - The trust wire contracts (`trust-job-payload.schema.json`, `trust-result-contract.schema.json`, `validate-trust-result.cjs`) are registered in `contracts.manifest.json`, so `check:contracts:integrity` digest-checks them and the bounded-schema/custody-field guards apply to them like every other CertOps contract. The integrity check now also rejects any manifest entry whose `status` it does not know how to verify, instead of skipping it and still reporting success, so a registered contract can no longer sit unverified without the check failing.
  - An execution-enabled agent advertises `distribute-trust`/`revoke-trust` among its claimable actions, and the agent-protocol wire schema's `supportedActions` enum admits both values, so the control plane's claim query and the `trust-anchor-deploy-v1` capability-freshness gate dispatch these jobs to real agents.
  - Releasing a reference that is not the last one updates only that owner's row to `removed` and creates no job at all, so one owner's `revoke-trust` never removes material another owner still depends on. Whether the material is physically removed is decided from the tuple's full provenance history, not the releasing row alone, and a reference that was only ever `preexisting` is released without dispatching a removal that would fail with `receipt_missing`.
  - An interrupted install is recoverable: if the agent crashes between writing its pending-install receipt and completing the OS mutation, its next attempt for that tuple re-probes the OS store and reclaims the stale receipt when the fingerprint is genuinely absent, rather than refusing that (store, fingerprint) forever. A retry after a failed platform trust-refresh re-runs the refresh instead of treating its own half-written state as preexisting.
  - **Operator guidance:** if a `distribute-trust`/`revoke-trust` job never leaves `pending`, check the target agent's declared capabilities first; the control plane will not assign the job until that agent has advertised `trust-anchor-deploy-v1`, and the job simply waits rather than failing loudly in the meantime. Also note that revoking one `owner`'s reference to a trust anchor never removes the certificate from the agent's OS trust store on its own if another `owner` still references that same anchor on that same agent; the certificate is only actually removed once the last live owner reference is released. If a job reports `receipt_finalize_conflict`, the OS mutation did succeed and the conflict is recorded in `last_error`; see `docs/certops/agent.md` section 9 for the recovery path.
...
- New evidence event types `trust.distributed` and `trust.revoked`, and new audit events `CERTOPS_TRUST_ANCHOR_DISTRIBUTED`/`CERTOPS_TRUST_ANCHOR_REVOKED`, fired only on a trust-anchor job's own successful terminal transition (a terminal failure still reuses the existing generic `CERTOPS_JOB_FAILED` event, carrying the same trust-result fields so a failed job's audit trail is no less informative than a successful one's). All seven trust-anchor audit actions carry the anchor's name, fingerprint, and host alongside the IDs, and the dashboard's Audit page has a dedicated formatter and filter entries for them.
- The dashboard's manual job-creation modal no longer offers the seven execution-override fields (`commandRef`, `caEndpoint`, `target`, `sans`, `certPath`, `dnsZone`, `dnsProvider`) for a `renew` operation, matching the renewal-override restriction above.
...
- The CertOps post-migration integrity check (`certops:integrity-check`) failed its capability-audit assertion for every agent that registered and never subsequently changed its declared capabilities. Registration always writes a `declared_capabilities` value and audits it as `CERTOPS_AGENT_REGISTERED`, but the assertion only accepted `CERTOPS_AGENT_CAPABILITIES_CHANGED`, so a perfectly healthy agent counted as missing its audit trail. The assertion now accepts either action, matching its documented intent of proving that some audit trail exists per agent. This was masked in CI because the check runs against a freshly seeded database with no registered agents.
- Agent stdout/stderr log lines had no timestamp, and the outbox-transmission-failure and lease-renew-failure lines carried a stray double `tokentimer-agent:` prefix (they read `tokentimer-agent: tokentimer-agent: ...`). Every line now starts with an ISO-8601 UTC timestamp, the prefix is added once by the shared logger at every call site, and the job-finished line now includes `errorMessage`/`failureCategory` when the job failed instead of only the bare status word.
- `certificate_jobs.result_metadata` was left at its insert-time default (`{}`) for every deploy job, and no `certificate_job_log` row was ever written when an agent reported a real result. Agent-result ingestion now persists the reported result fields (`rejectionReason`/`keyRotated` for every job family, plus the full trust-result contract for trust-anchor jobs) into `result_metadata`, and inserts a matching `certificate_job_log` row for the completion/failure event.
- The agent's local outbox now backs off exponentially (15s, 30s, 60s, ... capped at 30 minutes) between retries of a repeatedly-failing entry, instead of retrying it on every poll tick forever, so a stuck entry stops flooding the log while it keeps retrying at a slower rate. A failure the agent can classify as permanent (a 4xx other than 408/429/401/403) is quarantined into a `dead-letter` subdirectory immediately; an unclassified transient failure is quarantined only as a last resort, after at least 8 real delivery attempts AND 48 hours of retrying measured from the entry's first attempt (not from when the underlying job finished, so an agent that was powered off for a week still gets a full retry window rather than being dead-lettered on its first post-restart failure). Both directories are retention-capped (5000 entries / 7 days for the main outbox, 5000 entries / 30 days for dead-letter) and pruned on a rate-limited cadence from the poll loop; dead-letter retention age is measured from the moment an entry was actually quarantined (`quarantinedAt`), not from the underlying job's completion time. Entries too corrupt to parse are aged off their file mtime, so a poisoned file cannot occupy the directory indefinitely. Dead-letter has no automatic replay path: inspect `dead-letter/` if a job's result never reaches the control plane.
- `@tokentimer/contracts` now declares `ajv`/`ajv-formats` as its own runtime dependencies instead of relying on a sibling workspace package's copy. A production, workspace-filtered install (`pnpm install --prod --filter @tokentimer/api...`, as the release Docker images use) does not carry a dependency's own transitive `require`s into that dependency's resolution scope unless the dependency declares them itself.
