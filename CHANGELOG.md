...
- **Windows agent execution surface, groundwork (CertOps).** Signed job dispatch, a service-aware Windows host with ACL-protected agent credentials, diagnostic-agent isolation, node-free reference clients, and the schema and storage groundwork for IIS and Windows certificate-store targets, delivered as a stack of PRs. These PRs shipped the signed-dispatch and schema groundwork first; the CNG/IIS executors and capability qualification landed later in the same 0.12.0 cycle (see the Windows/IIS execution surface entry below), so a job is never dispatched to a path that cannot run it. The PRs:
  - **Signed dispatch, dual-format (#118).** Jobs are canonicalized exactly once server-side; both the original v1 wire shape and a new v2 "exact-byte signed envelope" (`envelopeVersion: 2`, `payloadB64`/`signatureB64`/`signingKeyId`) sign the same canonical bytes. v2 lets clients verify the signature over the raw decoded bytes before ever parsing JSON (a "two-decode gate"), sidestepping client-side JSON canonicalization entirely. Selected per job based on the agent's *fresh* capability declaration (`capabilities_updated_at` within `CERTOPS_CAPABILITY_FRESHNESS_MS`, default 10 minutes); an agent that stops heartbeating reverts to v1 automatically. See ADR-0003's addendum.
  - **`declaredCapabilities` on heartbeat, not just register (#119).** An in-place agent upgrade can now advertise a newly-supported capability without re-enrollment (three-valued: omitted preserves, `[]` clears, non-empty replaces). Requires upgrading the server before any agent sends the field.
  - **Windows service host and ACL enforcement (#117).** A native Go SCM-aware host (`packages/agent/windows-service-host`) answers the Service Control Manager's start/stop/interrogate protocol on the agent's behalf, since a plain Node process cannot; `install-agent.ps1`'s `binPath` now points at this host instead of `node.exe` directly. The registry-persisted bootstrap token is scrubbed from the service's `Environment` value once registration succeeds.
  - **Required `agentId` in signed payloads (#121).** `agentId` is now a signed, staged field: stamped into every dispatch, enforced additively behind `CERTOPS_AGENT_REQUIRE_SIGNED_AGENT_ID` and a matching agent-side `checkAgentIdBinding` gate (absence-tolerant until the effective flag flips). The `agent-id-binding-v1` capability is advertised only once enforcement is actually effective for that agent, not merely because the release defaults to it.
  - **Diagnostic-agent isolation (#122).** A dedicated `protocol_smoke` job operation and an immutable, server-assigned `certops_agents.agent_kind` (`normal`/`diagnostic`) let an operator verify the signed-dispatch pipeline end-to-end through a session-authenticated, single-use `POST .../certops/agents/diagnostic-bootstrap` route, without a diagnostic run ever being able to claim real certificate work. Orphaned diagnostic agents are retired transactionally by a worker sweep.
  - **Node-free reference clients (#124).** `tokentimer-protocol.sh` (Bash + curl + jq + OpenSSL 3) and `tokentimer-protocol.ps1` (PowerShell 5.1/7) exercise the real agent protocol without a Node dependency, using a small signed Go Ed25519 verify-only module as their only cryptographic implementation.
  - **Windows/IIS target schemas and trust-anchor persistence (#125).** Nullable `windows_store`/`windows_site`/`windows_port`/`windows_sni_host` columns and a `windows-iis` target type; `certops_trust_anchors`/`certops_trust_anchor_installations` storage groundwork for CA root/intermediate distribution, gated behind a build-embedded qualified-capabilities manifest.
  - **Claim/lease/attempt visibility on job-detail reads (#112).** `claimId`, `claimedByAgentId`, `claimedByControllerClusterId`, `claimedByAgentSigningKeyId`, `leaseExpiresAt`, `leaseRenewedAt`, `attemptCount`, and `maxAttempts` are now surfaced on the job timeline for workspace managers/admins; `claimId` (a reusable lease credential, not just informational metadata) is redacted for everyone below admin/owner.
  - **QA infrastructure for the envelope-v2/Windows-groundwork surface (#123).** Deterministic golden fixtures for envelope-v2 (SHA-256-seeded PRNG, fixed clock), a `fast-check` fuzz target against the v2 decoder, a generated skip-inventory (`docs/certops/skip-inventory.generated.md`, categorized `no-host`/`unimplemented`) with a drift guard, centralized coverage floors requiring a `why:` comment on any threshold change, a post-migration data-invariant check (`certops-integrity-check.cjs`), and a redaction test proving agent-controlled free text (e.g. `errorMessage` on `CERTOPS_JOB_FAILED`) is scrubbed before it reaches the audit trail.
  - **Every fail-closed guarantee in the v2 envelope protocol named in a dedicated test file (#126).** No behavior change; consolidates and labels the existing two-decode-gate/signature/bounds assertions so a future regression in any one of them fails with a specific, named test rather than a generic parse error.
  - See `docs/certops/agent.md`'s new wire-contract compatibility matrix for upgrade ordering across these changes, `docs/adr/0012-certops-windows-execution-surface-and-trust-anchors.md` (#120) for the full design record, and `ROADMAP.md`.
- **Operator audit-SQL advisory and a heavier CI tier (#127).** `pnpm run certops:audit-nonce-ledger` flags nonce-ledger rows whose completing agent doesn't match the agent the dispatch nonce was issued to, joined on `job_id`/`workspace_id` — a mismatch that (absent a code bug) should never happen, so a hit is worth a human look. `pnpm run verify:deploy` adds a second, heavier verification tier that runs the full suite (unit, integration, e2e agent) against a live Docker Compose stack rather than only unit-level mocks, folded into `verify`.
- **Enabled two previously-skipped auto-sync enterprise edition-gate integration tests (#128).** Both carried a `no-host` skip reason with a note that they were safe to re-enable once run against a live Docker Compose stack; verified against one and un-skipped, dropping the generated skip inventory from 19 to 17. No behavior change.
- **Agent health, downtime alerts, and renewal-path visibility (CertOps, #151).** Per-workspace agent alert settings, live liveness computation, worker-cron downtime email delivery when an agent goes stale or offline, job-eligibility gating that refuses to dispatch renewals an agent cannot run, and renewal-path health evaluation surfaced as a badge on certificate instances and the Agent Fleet panel (dependent-certificate counts, OS display). Migrations 45-46 add the observation/downtime-alert schema. Real-host verified against a Windows Server VM through the live API.
- **Windows/IIS execution surface: CNG-native key custody, IIS binding, retention, and discovery (CertOps, #148).** The executors the groundwork entry above deferred, plus their wiring into the real renewal pipeline: a `renew` job whose custody is `os-store-managed` against a `windows-iis` target now runs the CNG-native path end to end (key generated inside the Microsoft Software Key Storage Provider so the private key never touches disk, ACME order, `certreq -accept` enrollment, `netsh http` IIS binding rebind with automatic rollback to the prior binding on a failed verify, and a claim-bound verify-evidence report carrying the deployed certificate's fingerprint and validity window so the control plane can reconcile it exactly like a file-based deployment). A restart-safe, agent-local retention ledger records the certificate a binding previously pointed at and defers its cleanup until every eligibility condition (no remaining binding or key-container reference, a successful replacement handshake, a deadline) holds, retrying deferred rows on later sweeps rather than losing them. An abandoned CNG key container from a failed ACME order (rejected, timed out, or the process crashing mid-order) is best-effort deleted rather than leaked. Observe-only discovery reports what is in a Windows machine certificate store and what `http.sys` bindings reference, using only `certutil`/`netsh`'s own text output and never touching key material. `windows-cert-store-v1` and `iis-binding-v1` are now qualified and advertised (`trust-anchor-deploy-v1` stays gated: no executor yet); `os-store-managed` key custody is now reported as agent-deployable. All of the above, including the full agent-install-through-auto-renewal-through-retention-sweep loop, was verified end to end against real Windows Server hosts (a real CNG store, a real IIS binding, and a real ACME order against a local test CA), not just unit-tested against mocks. Pre-merge code review found and closed five more gaps in the same surface before it ever shipped: the store-scoped mutex (ADR-0012 decision 13) now detects and steals a lock left behind by a crashed or killed process (a dead PID, or simply older than 30 minutes) instead of blocking every future renewal against that store forever; the abandoned-key cleanup above now also covers a bare `certreq -accept` failure, not only a failed ACME order; the mandatory delete-then-add IIS rebind now reads back and reapplies the outgoing binding's revocation-checking, CTL, DS-mapper, and client-certificate-negotiation settings instead of silently resetting them to `netsh`'s defaults on every renewal; the retention sweep described above is now actually invoked periodically by the running agent (`windows.sweepIntervalMs`, default 6 hours), not merely reachable in isolation; and a non-SNI (`ipport=`) binding is now understood — and surfaced as a non-fatal `precedenceWarning` on deploy — to always take precedence over an SNI (`hostnameport=`) binding on the same port for a client connecting to that address, a real `http.sys` behavior this module cannot override, only detect. A second review pass closed four more gaps: a CNG key container abandoned by a crash between `certreq -new` and `certreq -accept` is now freed automatically by a startup reconciliation sweep instead of left behind indefinitely; superseded-certificate retention now requires a persisted issuance record, not just a container-naming match, before treating a predecessor's key container as this agent's own to delete, so a human operator's own `certreq` enrollment is never mistaken for agent-owned material; the non-default-store `certutil` mirror path now ends with an explicit confirmation query instead of trusting the prior exit codes alone; and the dashboard's deploy-agent modal now offers a Windows install command alongside the existing Linux one. A third review pass found and closed a release-blocking gap in that same crash-reconciliation sweep, plus three more: the sweep previously queried only the job's recorded target store before freeing an "orphaned" container, but `certreq -accept` always lands in the default `My` store first, with a mirror into a non-default target store (e.g. `WebHosting`) happening as a separate, later step — a crash between those two steps left a certificate genuinely using the key sitting only in `My`, which a target-store-only check would miss, incorrectly deleting a live private key; the sweep now queries both stores and fails closed on either query error. Superseded-certificate retention's ownership-provenance record is now upgraded, immediately after a successful `certreq -accept`, with the actual accepted certificate's thumbprint, and requires that thumbprint to match the predecessor certificate before granting `tokentimer_installed` provenance — closing a gap where a differently-installed certificate in an agent-created container could otherwise inherit that provenance from the container's mere existence. The mandatory delete-then-add IIS rebind now additionally preserves the newer Windows Server 2019+ per-connection policy flags (`reject`, `disablehttp2`, `disablequic`, `disablelegacytls`, `disabletls12`, `disabletls13`, `disableocspstapling`, `enabletokenbinding`, `logextendedevents`, `enablesessionticket`, `disablesessionid`), alongside the eight classic Server 2012+ settings the previous pass already covered. The SNI-precedence warning now also detects a shadowing non-SNI binding on a concrete IP, not only the two wildcard address forms. A fourth review pass found and closed two more gaps: normal (non-crash-recovery) execution against a non-default target store was locking only that target store, while `certreq -accept` always mutates the default `My` store first regardless of target — a `WebHosting`-targeted renewal could therefore race an unrelated `My`-targeted job's own concurrent mutation of `My`; execution now acquires both stores' locks (deduplicated for the common `My`-only case) for the duration of the operation, via the same store set the crash-reconciliation sweep already used, so the two paths can never deadlock against each other. The IIS rebind's `disablelegacytls` preservation added in the third pass never actually matched a real binding, because Microsoft's own `netsh http show sslcert` reports this one field with a different vocabulary than every sibling per-connection flag (`Set`/`Not Set`, not `Enabled`/`Disabled`/`Not Set`) — an outgoing binding with legacy TLS genuinely disabled had that restriction silently dropped, not merely reset, on every renewal; the parser now recognizes both vocabularies, with a regression test built from Microsoft's own documented output shape. See `docs/certops/agent.md` for the updated platform matrix and `docs/adr/0012-certops-windows-execution-surface-and-trust-anchors.md` for the design record.

### Fixed

- **`install-agent.sh`'s hardened unit failed on every single fresh Linux install until manually worked around.** The systemd drop-in binds every `--write-path` into `ReadWritePaths=`, but the installer never created the directory first, so the very first `systemctl start` after install reliably failed 4-5 times with `Failed to set up mount namespacing: <path>: No such file or directory` (`status=226/NAMESPACE`) before an operator manually created and chowned it. Reproduced on every distro real-host-tested this cycle (Ubuntu 22.04/24.04/26.04, AlmaLinux 9). `install-agent.sh` now creates, owns, and permissions (`0750`) each `--write-path` directory itself, right after the state directory, before the unit is written.
- **`acmeKind: "acme.sh"` dispatch now always passes `--dnssleep 0`.** The ACME adapter's `--signcsr` invocation previously let acme.sh run its own generic, public-resolver-bound DNS propagation recheck after `certops-dns-hook`'s own authoritative-resolver wait (`waitForTxtPresent`) had already confirmed the TXT record. That recheck can never succeed for a zone with no public delegation (an internal-only lab/test domain), so it retried for minutes and failed the whole run even though the record was already correctly verified. `buildAcmeShArgs` now unconditionally adds `--dnssleep 0` to skip acme.sh's redundant second check in favor of the one the hook already did; this changes behavior for every `acmeKind: "acme.sh"` dispatch, not only the lab rig that surfaced it, though `certops-dns-hook`'s own authoritative check remains the actual DNS-01 verification gate either way.
- **Discord release announcements silently stopped after v0.10.3 (#111).** The `notify-discord` job (added in `release.yml` on `main` by #97) was overwritten when the long-lived `release/0.11.0` branch (#96) was based on a pre-Discord snapshot of `release.yml` and squash-merged, deleting the job with no diff review catching it — every release from v0.11.0 through v0.11.3 shipped without a Discord post. Re-added the notification as its own workflow, `.github/workflows/discord-release-notify.yml`, triggered by the repo's `release: published` event (fired by `gh release create`) instead of a job inside `release.yml`, so it can no longer be silently dropped by a `release.yml` merge and correctly never fires for beta dry-run tags (which never get a GitHub Release). Added `.github/workflows/discord-release-backfill.yml` (`workflow_dispatch`, takes an `after_tag` input) to post any run of missed releases going forward, and used it once to backfill v0.11.0 through v0.11.3.
- **Nested-compression bypass in the private-key-material detector (#131).** `classifyCompressedBuffer` decompressed one recognized gzip/zlib layer and scanned the result, but if that result was itself gzip/zlib-shaped it was returned as scan-ready plaintext without a second bounds check, letting nested compression hide key material below the outer layer's ratio/size ceilings. Now fails closed whenever decompressed output itself looks compressed.
- **Missing regression test for `existingKey` zeroization on deploy failure (#129).** `deployCertificateAndKey` zeroized the previous live key on a failed atomic write, but no test proved it; added one that mocks a downstream `rename` failure and asserts the actual buffer bytes are zero afterward.
- **CertOps sub-nav tabs had no visible keyboard-focus indicator (#130).** `CertOpsSubNav` styled `_hover` and `_activeLink` but never `_focusVisible`, unlike every other interactive control in the dashboard, so keyboard users got no indication of which tab was focused. Added the same `_focusVisible` outline-ring pattern already used elsewhere; also strengthened an `EvidenceTimeline` test to assert an unrecognized event type renders a bounded fallback label rather than leaking raw metadata.
- **`CopyableId`'s monospace id chip could overflow its container at narrow widths.** The signing-key-id format (`ttsk_` plus 16 hex characters, no hyphens) has no natural line-break point, so on a narrow viewport (roughly 320-360px, e.g. `EvidenceTimeline` inside a phone-width dashboard) it forced the chip past the card edge instead of wrapping, while hyphenated UUID-shaped ids in the same component wrapped fine. Found via a real-browser rendering pass (Playwright, both light and dark color modes, three viewport widths) driven against production fixture data rather than static review. Added `overflowWrap='anywhere'` to the id `Text`; verified the same pass no longer reports horizontal overflow at any tested width.
- **`react-router` reintroduced at 8.3.0 by a Dependabot bump, incompatible with the dashboard's React 18.3.1 pin (#135).** The exact incompatibility PR #108 had already reverted once resurfaced through a routine dependency bump caught up in the CertOps merge batch, breaking 19 of 39 dashboard test files (`useOptimistic` is React-19-only). Reverted to `7.18.1`.
- **Linux agent service had no core-dump suppression (#133).** `packages/agent/scripts/tokentimer-agent.service` never set `LimitCORE`, so a crashing agent process could leave a core dump containing decrypted key material on disk (soft/hard limits inherited the ambient default rather than the `0`/`0` ADR-0012 requires). Verified live against a running systemd process's `/proc/<pid>/limits` before and after. Added `LimitCORE=0` to the unit file and a new static guard, `scripts/ci-guards/core-dump-suppression.cjs`, so a future edit dropping the directive fails CI instead of shipping silently.
- **Dashboard asset search reset filters and re-rendered on every keystroke (#134).** The debounce lived downstream of the input, in `App.jsx`, delaying only the derived value used for filtering while every keystroke still updated `panelQueries` and re-rendered the dashboard immediately. Moved the debounce into a new `AssetSearchInput` component in `AssetFilters.jsx`, which owns a local draft value and commits a single update upstream after a 300ms pause.
- **Agent verify probe could fail a good deploy on multi-worker reloads (#143).** `runDeployReloadVerify` probed the live endpoint immediately after a service reload returned success, but on multi-worker services (nginx with long-lived keepalive connections, blue/green load balancers) the reload command can return before every worker has cut over, so the probe connected fine but observed the previous certificate's fingerprint and the agent failed an otherwise-successful deploy. Reproduced empirically against a live nginx reload: a probe run ~9ms after reload reliably saw the old fingerprint, one run ~1500ms after reliably saw the new one. Added `verifyDeployedCertificateWithRetry`, a bounded retry-with-backoff wrapper (4 attempts, 250/500/750/1000ms) that retries only on a connected, real-fingerprint mismatch — never on connect failures, timeouts, "no certificate presented," or an authorization/policy rejection.
- **IIS target-type migration dropped `agent-host` from the CHECK constraint (#147).** Adding `windows-iis` rewrote the `certificate_targets.target_type` CHECK without preserving `agent-host`, so inventory upserts for agent-host targets would fail after upgrade. Restored the complete union.
- **OpenAPI claim `supportedActions` enum lagged the agent-protocol schema.** After `protocol_smoke` was added to `agent-protocol.schema.json` for diagnostic agents, OpenAPI still listed only `renew`/`deploy`/`reload`/`revoke`/`noop`, breaking the contract mirror test. Added `protocol_smoke` to the OpenAPI enum.
- **Stale-agent health sweep failed with Postgres `inconsistent types deduced for parameter $1`.** The incident INSERT reused `$1` both as a UUID column value and as `$1::text` inside the `down_alert_key` expression; the alert key is now built in application code and bound as `$4`.
- **Renewal-path real-topology integration fixtures seeded incomplete `renewalProfile` objects.** Every case then short-circuited to `unknown` via `validateRenewalProfile`. Executable cases now seed a full schemaVersion-1 profile plus matching agent capability arrays (`supported_operations` / DNS / command profiles / target selectors).
- **Controller observation envelopes could flake on `Private key material is not accepted`.** The private-key detector hex-decoded SHA-256 digests (idempotency keys, fingerprints) and occasionally treated zlib-looking random bytes as compressed key material. Hex now only matches concrete DER/PKCS#12/JKS shapes, and `createObservationEnvelope` scans the caller payload before attaching derived hex keys.

### Security

- **Node.js base images bumped 22.23.0 → 22.23.2 (#158).** Clears Grype High findings CVE-2026-56846, CVE-2026-56848, CVE-2026-58043 (and related Medium/Low Node CVEs) against the API/dashboard/worker/controller images.
- **`js-yaml` workspace override bumped 4.3.0 → 4.3.1 (#156/#157).** Patches CVE-2026-59870 / GHSA-5p4m-2wfm-xmqj (quadratic CPU consumption in `!!omap` resolution). Dependabot's direct-specifier bump alone was insufficient because `pnpm-workspace.yaml`'s override still forced 4.3.0; the lockfile-overrides CI guard was synced in the same pass.
...
- **Agent state directory: ACME subdirectories were not hardened to `0700`.** `install-agent.sh` only applied `chmod 0700` to the top-level `state/` directory; the `state/acme/certbot/*` and `state/acme/acme.sh/*` subdirectories it creates afterward inherited the ambient umask (`0755`) instead. This was harmless in practice only because the parent `state/` directory already blocks traversal for non-owner users, but it left the ACME subtree one permissions change away from being readable if `state/` itself were ever loosened. The installer now runs `chmod -R go-rwx` on `state/acme` right after creating it, so the `0700`-everywhere invariant holds on its own rather than depending on the parent directory as the only line of defense. Found during CertOps 0.11.1 manual production-readiness testing on `tokentimer-cloud`.
- **Fleet `compatibilityState` showed `outdated` for every agent, regardless of how current it actually was.** The `outdated` heuristic compared an agent's build version against `CERTOPS_AGENT_MAX_AGENT_VERSION`, whose default (`99.999.999`) is an intentionally unbounded reject-ceiling meant only to stop the control plane from ever blocking an agent for being newer than the server knows about. Reused as the "latest version" reference for the `outdated` label, that sentinel's major version (`99`) is always ahead of any real agent's (`0.x`), so `computeAgentCompatibility` flagged `outdated` unconditionally by default, unless an operator manually set and continuously re-bumped `CERTOPS_AGENT_MAX_AGENT_VERSION` on every release, which no deployment (including `tokentimer-cloud`) did. Introduced a distinct `CERTOPS_AGENT_LATEST_KNOWN_VERSION` reference, defaulting to the shipped agent package's own version (`packages/agent/package.json`) so it tracks every release automatically; `CERTOPS_AGENT_MAX_AGENT_VERSION` keeps its original, unbounded reject-ceiling role. Found while investigating why `tokentimer-cloud`'s agent fleet panel showed every agent as "Outdated."

## [0.11.1] - 2026-07-29

### Fixed

- **Release CI could not publish the Helm chart** — the `release.yml` workflow's packaged-chart smoke test (`helm lint` / `helm template` against the built `.tgz`, run twice: once right after packaging, once again in `verify-release`) never passed a value for `config.adminEmail`, a chart-required field. Every tag push failed at that step, which blocked `verify-release`, `publish-helm-chart`, `create-release`, and `promote-latest` from ever running. Both steps now pass the same `ci@example.com` placeholder already used by `scripts/helm-template-verify.sh` and `deploy/helm/ci/ci-values.yaml`, and `helm-template-verify.sh` gained a packaged-chart parity check so this exact failure mode is caught locally and in `ci.yml` before a tag is ever pushed.
- **v0.11.0's Helm chart was never published** — the Docker images and agent tarball for v0.11.0 built and were verified successfully, but the release workflow failure above meant the Helm chart was never pushed to `oci://ghcr.io/tokentimerch/charts/tokentimer` for that tag. Self-hosters installing via Helm should skip `v0.11.0` and install `v0.11.1` or later; the Docker Compose and manual-manifest install paths for `v0.11.0` were unaffected.

## [0.11.0] - 2026-07-28

**Do not install this release via Helm.** The Helm chart was not published for `v0.11.0` due to a release-pipeline defect (fixed in `v0.11.1`, see above); `helm install`/`helm upgrade` against this tag's OCI chart will fail to resolve. Docker Compose and manual-manifest installs are unaffected. Use `v0.11.1` or later for Helm installs.
...
- **TokenTimer Agent** (`packages/agent`) — an outbound-only execution-plane process that registers against the control plane, polls for signed work, enforces agent-local policy, and reports schema-valid results and evidence:
  ...
  - Native DNS-01 solvers for twelve providers (Cloudflare, Route53, Google Cloud DNS, Azure DNS, RFC 2136, acme-dns, OVHcloud, Hetzner Console/Cloud DNS, Infomaniak, Exoscale, PowerDNS, and the test-only `pebble-challtestsrv` used to exercise the DNS-01 path against a local ACME test CA without a real DNS provider account) plus a guided onboarding surface.
- **Control-plane agent backend** — the server-side counterpart: agent bootstrap-token registration, heartbeat/claim/result machine routes, Ed25519 job signing service (shared canonical JSON), per-agent monotonic sequence enforcement (defense in depth against replayed or out-of-order agent messages), approval gates (non-requester rule, canonical payload-hash binding, claim-side invalidation on payload edits), `cert_renewal_failed` alerting on terminal renewal failures, a maintenance worker (lease reaper, stale-agent and nonce sweeps, renewal scheduler, outbox drain), per-CA renewal caps, and a bulk-renewal endpoint.
...
- **First-time issuance (`issue` job operation)** — a control-plane operation for a certificate TokenTimer does not track yet. Previously the only way to issue was a bare `renew` job with no subject: the agent ran a real ACME order and deployed a genuine certificate, and nothing was recorded, so the operator got no inventory row, no expiry tracking and no renewal. An `issue` job creates the managed certificate up front at status `provisioning` (source `agent_issuance`) in the same transaction as the job, then reconciles it to `active` with real x509 metadata from the agent's own verify evidence. It requires an `idempotencyKey` (a retry returns the original job rather than placing a second ACME order), rejects a caller-supplied `subjectId`/`payload.certificateId`, and shares the per-CA renewal cap. The wire protocol is unchanged: signed dispatch translates it to the existing `renew` action, so there is no schema or `schemaVersion` bump. Agents do have to declare the `evidence-claim-binding-v1` capability to be offered issuance work (below). See `docs/adr/0008-certops-upfront-issuance.md`.
- Agent verify evidence now carries the deployed certificate's serial, validity window, subject, issuer and DNS SANs, which is what lets the control plane reconcile a provisioned certificate without ever reading key material.
...
- **Renewal automation control surface** — deriving a renewal profile at issuance (below) made automatic renewal live for everything TokenTimer issues, and initially made it unconditional: a profile appeared by itself, the scheduler acted on it, and nothing in the product could inspect or stop it. Four routes and a page close that. `GET .../certops/profiles` and `GET .../certops/profiles/:profileId` list and read renewal profiles with the certificate count each covers, `GET .../certops/renewals/upcoming` returns the schedule the sweep is working from (soonest expiry first, with each certificate's renewal window and last attempt), and `PATCH .../certops/profiles/:profileId` edits a deliberately narrow subset behind the new `certops.renewal_profile.manage` permission (workspace admin). The dashboard's **Renewal automation** page (`/certops/renewals`, with its own nav entry) renders both panels and exposes exactly two controls: an automatic-renewal switch and the renewal lead time.
  - **An off switch.** `certificate_profiles.status` is now read as "is automatic renewal on": `disabled` and `archived` exclude every certificate linked to the profile, reported as the `skipped_auto_renew_disabled` sweep outcome and as a `disabled` renewal state per certificate. Expiry monitoring and alerting keep running, so switching renewal off silences the renewal and not the warning. `archived` is deliberately not settable through the API.
  - **Deployment details are immutable.** `acme`, `ca`, `dns`, `target` and `deploymentTargets` are refused with `422 CERTOPS_PROFILE_FIELD_IMMUTABLE` naming the fields. Those values are trustworthy precisely because a real ACME order proved them against a real host, so repointing where a live certificate is written is a re-issuance rather than a settings change. Editable fields are the ones that cannot change what executes on a host: SAN policy, key algorithm and size, key rotation policy, verification, preferred chain, lead time, and the switch. Every write revalidates through `validateRenewalProfile`, the same gate the scheduler admits on, so the API cannot store a profile the scheduler would later refuse.
  - **Reads are manager-gated, not viewer-readable.** A profile body is deployment topology rather than expiry metadata: certificate and key paths, the reload unit, file ownership and modes, the ACME command reference, the CA account reference, the DNS zone. All three read routes therefore require at least `workspace_manager`, matching the agent and machine-token routes rather than the certificate inventory. A viewer keeps full visibility of what expires and whether it auto-renews, through the inventory and the renewal badge, without gaining a map of where each key sits on which host.
  - **No create and no delete on the profile-management routes**, because a derived profile exists only because an issuance produced it; `PATCH` never creates one. A certificate TokenTimer did not issue can still be brought under automatic renewal through the separate adoption flow below, which derives a profile for it explicitly rather than through `PATCH`.
  - Switching renewal off asks for confirmation and states how many certificates the profile covers; switching it back on does not. ADR-0010 Amendment 1 records the reasoning.
  - **The schedule reports what will not renew, not just what will.** `GET .../certops/renewals/upcoming` lists every renewable certificate in the workspace and labels each one the scheduler will skip with a `blockedReason`: `auto_renew_disabled`, `no_profile`, `incomplete_profile` or `unknown_expiry`. Readiness is decided by calling the same profile resolver the sweep admits on, so the page cannot claim a certificate is covered when the scheduler would refuse it. Restricting the list to certificates that do renew (which an earlier revision did, by joining profiles) inverted the page's purpose: the certificates omitted were exactly the ones that will silently expire, so a workspace where nothing renewed rendered as an empty, reassuring schedule. An empty table now means there are no renewable certificates at all. The distinction between an operator's deliberate switch-off and a broken profile is kept in the UI too, because only the first is fixable from that page.
- **Renewal adoption for discovered and imported certificates** — the control surface above only lets an operator manage a profile an issuance already derived; it had no path for a certificate TokenTimer never issued (an `agent_filesystem` discovery, a `cert_manager` observation, an import). `POST .../certops/certificates/:certId/renewal-setup` derives a profile for such a certificate from an operator-supplied ACME/DNS configuration, subject to the same preconditions the scheduler itself would check (a known `deployed_cert_path`, no existing profile, the certificate not already retired); `DELETE .../certificates/:certId/profile` detaches it again. Because deriving a profile is not instantaneous (it goes through the same outbox as the alert-intent pipeline), the certificate exposes a `renewalSetup` projection (`none` / `waiting` / `configured` / `skipped` / `failed`, with a machine-readable outcome code) so the dashboard can show progress rather than a certificate that silently never got a profile, and a `POST .../renewal-setup-intents/:outboxId/retry` route for a `failed` intent. The Certificates page surfaces "Set up renewal" / "Detach" / "Retry" actions per row once a certificate is eligible.
...
- The CertOps maintenance worker gained a sixth sweep, `outbox-drain` (`CERTOPS_SWEEP_OUTBOX_DRAIN_ENABLED`). It is what turns a recorded renewal failure into a delivered alert, so it is **not optional for anyone relying on renewal-failure notifications**. Disabling it leaves intents accumulating as `pending` rather than losing them, but nothing is delivered in the meantime.
- ADR-0008 (upfront issuance) carries an inline amendment recording what live testing changed: renewal profiles are derived rather than operator-authored, `issue` now requires an agent capability, promotion requires claim-bound verify evidence, and a failed issuance deliberately does not alert. Two new records, `docs/adr/0009-certops-durable-side-effects-and-alert-policy.md` and `docs/adr/0010-certops-derived-renewal-profiles.md`, own the two decisions in full. ADR-0010 carries its own Amendment 1 for the control surface that derivation made necessary: the profile-level off switch, the immutability boundary on deployment details, and why there is no create route; Amendment 2 records the certificate-id-scoped profile naming above.

### Known limitations

- **The control plane itself still targets Linux with systemd only.** The CertOps agent is now a supported production deployment target on Windows Server (CNG-native key custody, real IIS certificate binding, and Windows certificate-store discovery — see the Windows/IIS execution surface entry above), but the API, worker, and scheduler side of CertOps is neither tested nor supported running on Windows.
- **The certops-agent registration/heartbeat/claim/lease/result routes ship in Core only.** Cloud and Enterprise pick these up when each variant repins to a Core release that includes them; a variant on an older Core pin, or one whose API layer does not mount `certops-agent.js`, will 404 on those routes even though Core documents and tests them.
