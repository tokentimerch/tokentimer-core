...
- **A fleet that renewed nothing looked identical to an idle one** - the renewal scheduler exported only its `created` count. All outcomes are now series on `certops_renewal_scheduler_certificates` labelled by `outcome`: `scanned`, `created`, `replayed`, `skipped_paused`, `skipped_ca_cap`, `skipped_incomplete_profile`, `skipped_not_agent_deployable`, `errors`. An observed-only certificate (no agent custody of its key, so it can never be agent-renewed) counts as its own skip rather than an error that would make every sweep look broken.
