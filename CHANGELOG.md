...
- **One certificate file could acquire two inventory identities** - an agent that issues to a path later scans that path and reports it as an ordinary filesystem discovery, and the two sources were keyed differently, so a duplicate row appeared. Issuance now records the deployed path and owning agent, and `(agent, path)` is the correlation key, so a later scan resolves to the existing row.
