...
- **A `windows-iis` target's `store` field was silently ignored by the executor.** `target.store` is a required part of the `windows-iis` target descriptor and was already threaded down to `acceptCertificateViaCng`, but that function never declared a `store` parameter, so the value was silently dropped and every certificate landed in the default `My` store regardless of what was configured, with no error surfaced anywhere. `certreq -accept` itself has no switch that targets a store other than `My` for a machine-keyset request, so a genuinely different `store` now goes through a `certutil -addstore`/`-repairstore`/`-delstore` mirror sequence that copies the accepted certificate into the requested store, re-associates it with its CNG key, and removes the `My`-store copy `certreq -accept` created. Requesting `store: "My"` (the common case) is unaffected and still touches only `certreq`. The non-default-store path is unit-tested against a stubbed `certutil` but not yet independently verified against a real Windows host; real-host verification against a non-`My` store (e.g. the `WebHosting` convention some IIS deployments use) is tracked as a follow-up. Retention (ADR-0012 decision 18) also gained ownership-provenance hardening in the same change: a certificate's key container is now recorded as agent-owned only when its name matches this agent's own naming convention, not merely because a key container is present, since a human operator's or a different tool's own CNG enrollment also reports one and must never be treated as eligible for automated cleanup.
...
- **Lost executable bit and shipped-agent regressions from the CertOps merge batch (#138).** `tokentimer-protocol.sh` (#124) lost its POSIX executable bit when rebuilt on a non-POSIX checkout, breaking `two-decode-gate` on the Linux CI runner; the shipped-sources scanner false-flagged a doc comment as an import and a vendored `zlib` require lacked the `node:` prefix the shipped-agent allowlist requires; and three previously-pinned dependency overrides (`ip-address`, `fast-uri`, `brace-expansion`) had newer advisories disclosed against the pinned versions. All fixed with no functional change to any shipped feature.
...
- **Prettier violation in the Audit.jsx capabilities-diff line from a manual merge-conflict resolution (#136).** Formatting only, no behavior change.
...
- **Agent verify probe could fail a good deploy on multi-worker reloads (VERIFY-RACE-01, #143).** `runDeployReloadVerify` probed the live endpoint immediately after a service reload returned success, but on multi-worker services (nginx with long-lived keepalive connections, blue/green load balancers) the reload command can return before every worker has cut over, so the probe connected fine but observed the previous certificate's fingerprint and the agent failed an otherwise-successful deploy. Reproduced empirically against a live nginx reload: a probe run ~9ms after reload reliably saw the old fingerprint, one run ~1500ms after reliably saw the new one. Added `verifyDeployedCertificateWithRetry`, a bounded retry-with-backoff wrapper (4 attempts, 250/500/750/1000ms) that retries only on a connected, real-fingerprint mismatch — never on connect failures, timeouts, "no certificate presented," or an authorization/policy rejection.
- **Reference clients could never hit their own decoded-payload-size check (#144).** ADR-0012's ninth amendment fixed an unreachable decoded-byte bound in the Node agent's signing module, but the Bash and PowerShell reference clients (`tokentimer-protocol.sh`/`.ps1`) were missed: their encoded-payload-char bound (65536) was the tightest value that still decodes to exactly the 49152-byte decoded bound, so no base64 string within that ceiling could ever decode to more bytes than the ceiling already permitted, making the decoded-byte check dead code in both implementations. Bumped both to 98304 to match the Node agent and the ADR, restoring real headroom so payloads in the 49153-73728 decoded-byte range are now actually caught by the decoded-byte check with its own rejection reason. No behavior change for any payload seen in production.
- **Windows/IIS discovery never reported a certificate's Subject Alternative Names, which machine store it came from, or which IIS site a binding belonged to.** `certutil -store` was invoked without `-v`, so its output carried no SAN extension section to parse at all, and nothing cross-referenced `http.sys` bindings against IIS site configuration. Discovery now runs `certutil -store <store> -v`, parses the SAN extension into `subjectAlternativeNames`, tags every returned certificate with the `store` it was read from, and best-effort resolves each binding's owning IIS site name by cross-referencing `appcmd list site`'s binding list against the binding's port/host-header/address (`boundSites`); a missing or failing `appcmd` never fails discovery itself, since site resolution is supplementary evidence, not part of the `certutil`/`netsh` contract discovery already relies on.
- **The dashboard Audit log under-reported evidence on a Windows/IIS renewal-unreconciled event.** `_ISSUANCE_UNRECONCILED`/`_RENEWAL_UNRECONCILED` events render only after the store/binding validation step already succeeded (only the later verify-evidence step failed), so the full `windowsStore`/binding fields are trustworthy and worth showing an operator investigating the mismatch. The Audit page's metadata formatter collapsed both that case and `RENEWAL_PROFILE_DERIVATION_DECLINED` (where a decline usually means the store/binding fields themselves failed validation, and showing them as fact would be misleading) into the same `targetType`-only rendering. Split into two formatters so the unreconciled events show their full Windows target detail again.
