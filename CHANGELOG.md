...
- **Obsolete-item cleanup on import now covers Vault, AWS, Azure Key Vault, Azure AD and GCP Secret Manager**, not just GitHub and GitLab, and the auto-sync worker uses the same cleanup path for every provider via a per-config `cleanup_obsolete` flag. Cleanup scope is now derived from a server-side scan record rather than from values the browser sends back, and only runs against sub-scopes the scan reported fully complete, so a truncated or partly-errored scan can no longer delete tokens it never actually checked. Tokens carry structured provenance (provider, instance, owner, kind), so two instances of the same provider (two Vault servers, two AWS accounts, two self-hosted GitLabs) can never delete each other's tokens. Each scan can drive at most one cleanup, and an older scan's cleanup cannot delete a token a newer concurrent scan just rediscovered. **Operator guidance:** tokens imported before this release are left untouched by cleanup where their originating instance and owner cannot be established from the old location string; rediscovering them inserts a fully attributed row rather than guessing at the old row's provenance. New migration (`50`) adds the scan tables and the token provenance columns.
