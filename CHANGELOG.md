...
- On Debian/RHEL, distributing a trust anchor now checks the actual anchors directory for a certificate with the target fingerprint under any filename, not only TokenTimer's own deterministically-named file. A CA that was already trusted under an unrelated filename (installed independently of TokenTimer, e.g. via a native `cp` into the anchors directory followed by the platform's own trust-refresh command) is now correctly detected and reported as already-present, instead of writing a duplicate copy, re-running the trust-refresh command, and misreporting a fresh installation the agent never actually performed. This mirrors the equivalent Windows check, which already enumerates the real certificate store rather than checking a single expected location.
- After a failed `update-ca-certificates` / `update-ca-trust` run, a retry no longer treats the on-disk anchor file as "preexisting" and skip the trust refresh. The agent now re-runs the refresh when its own receipt is still at `pending_install`, so a successful retry actually updates the effective trust bundle instead of reporting success while the store remains stale. The same fix now also applies to `revoke-trust`: resuming a `pending_remove` receipt on Debian/RHEL re-runs the platform refresh command rather than skipping it, so a retried removal is not falsely reported as complete while the CA is still trusted.
- Revoking the last reference to a trust anchor that was only ever reported as `preexisting` (TokenTimer never installed it and holds no ownership receipt) now releases the bookkeeping row without dispatching a physical removal job that would fail with `receipt_missing`.
- Re-approving an existing trust anchor fingerprint now updates `anchor_type` (and the stored PEM) when the caller submits a different type for the same certificate.
- Replaying a trust job with the same idempotency key no longer clears `last_attempt_at` or `last_error` on the installation row; an idempotent replay is side-effect free with respect to those fields.
...
- A retried `distribute-trust` that finds its own already-finalized receipt (agent restarted after a successful install but before its result reached the control plane) now promotes provenance to `tokentimer_installed` instead of leaving it at a state that would let a later revoke skip removing material TokenTimer actually installed.
- `revoke-trust`'s decision on whether releasing the last reference to a shared `(agent, store, fingerprint)` tuple should physically remove the material now considers that tuple's full provenance history, not just the releasing row's own provenance column, closing a gap where a second owner's release could leave the CA physically installed with no live reference to it.
- A trust job whose OS mutation genuinely succeeded but whose agent-local receipt-finalize write failed afterward (`receipt_finalize_conflict`) now settles the installation row on the observed outcome and records the conflict in `last_error`, instead of unwinding a mutation that actually happened. See `docs/certops/agent.md` section 9 for the operator recovery path.
- The succeeded-result contract for trust jobs now rejects a result naming its own action's failure-fallback outcome (`revoke-trust` reporting `installed`, `distribute-trust` reporting `already_absent`) on a job already classified succeeded, and requires a non-null `observedFingerprintAfter` whenever `distribute-trust` reports `installed`.
- Trust-job idempotency keys are now namespaced by operation and full target tuple (`trustAnchorId`/`agentId`/`store`/`owner`) rather than shared workspace-wide, so the same caller-supplied key reused for an unrelated operation or target no longer risks colliding with, or replaying, the wrong job. New migration adds `certops_trust_reference_release_idempotency` to back the no-op reference-release path under this same scoped key.
- The agent's local outbox now quarantines a transient (unclassified) delivery failure into a `dead-letter` subdirectory only after it has been retrying for 48 hours from creation, rather than after a small fixed attempt count (previously roughly 74-110 minutes); a permanent-error classification is still quarantined immediately. Both the main outbox and the dead-letter subdirectory are now retention-capped (5000 entries / 7 days for the main outbox, 5000 entries / 30 days for dead-letter) and pruned on a rate-limited cadence from the agent's poll loop. Dead-letter retention age is now measured from the moment an entry is actually quarantined (`quarantinedAt`, stamped on the entry at quarantine time), not from the underlying job's original completion time; previously an agent that had been offline longer than the dead-letter retention window could have its first post-restart delivery attempt quarantined and then deleted by the very next prune pass before an operator had any chance to inspect it.
- The dashboard's manual job-creation modal no longer offers the seven execution-override fields (`commandRef`, `caEndpoint`, `target`, `sans`, `certPath`, `dnsZone`, `dnsProvider`) for a `renew` operation, since the API resolves them from the certificate's stored renewal profile and rejects an override of any field other than `reason`.
...
- The agent-local `allowedCommands` policy gate that already refused `distribute-trust`/`revoke-trust` on Debian/RHEL when the platform's trust-store command wasn't allowlisted now applies on Windows too, under a new `trust-store:certutil` profile name. A renew-only Windows agent (no `allowedCommands` profiles configured at all) now refuses a trust-anchor job the same way its Debian/RHEL counterpart already did, instead of running it unconditionally.
