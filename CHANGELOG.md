...
- **Renewal automation control surface** — deriving a renewal profile at issuance (see Fixed, below) made automatic renewal live for everything TokenTimer issues, and initially made it unconditional: a profile appeared by itself, the scheduler acted on it, and nothing in the product could inspect or stop it. Four routes and a page close that. `GET .../certops/profiles` and `GET .../certops/profiles/:profileId` list and read renewal profiles with the certificate count each covers, `GET .../certops/renewals/upcoming` returns the schedule the sweep is working from (soonest expiry first, with each certificate's renewal window and last attempt), and `PATCH .../certops/profiles/:profileId` edits a deliberately narrow subset behind the new `certops.renewal_profile.manage` permission (workspace admin). The dashboard's **Renewal automation** page (`/certops/renewals`, with its own nav entry) renders both panels and exposes exactly two controls: an automatic-renewal switch and the renewal lead time.
  ...
  - **No create and no delete**, because a profile exists only because an issuance produced it. A certificate TokenTimer did not issue still cannot be brought under automatic renewal without issuing it through CertOps.
