...
- An asset (token, certificate, or CertOps agent) can belong to more than one contact group. Overlapping recipients get one live alert and one weekly digest for the unioned assets, not one message per group. Existing single-group assignments keep working. Multi-group writes are **on by default**. Migrations 52-54 add the join tables, backfill from `contact_group_id`, and rebuild join membership from the singular column (54 repairs stale join rows left by a 0.15.0 writer). Control Center eligibility unions every assigned group's thresholds and channels, matching discovery. **Operator action:** do not run a mixed 0.15.0 + 0.16.0 fleet. 0.15.0 never writes the join tables. After 54, a leftover 0.15.0 writer leaves stale membership and wrong alerts. `CONTACT_GROUP_PLURAL_WRITES=false` (Helm `config.contactGroupPluralWrites`) only blocks two-or-more `contact_group_ids` on 0.16.0; it does not make 0.15.0 write the join. Rolling a 0.15.0 binary back onto a post-54 database is unsupported. **Operator guidance:** write and read `contact_group_ids`. `contact_group_id` still works and returns the lex-smallest assigned group (or null for the workspace default). Generic digest webhooks keep `contact_group` and add `contact_groups`. Digest skip keys HMAC the destination with `WEEKLY_DIGEST_RECIPIENT_KEY` (else `SESSION_SECRET`). A failed digest recipient is not retried by the default Monday 09:00 job. Rerun the same week; `sent` claims make extra runs safe.
- Inventory, token and certificate details, and Control Center split **eligibility** (should an alert fire now) from **delivery** (what happened after queue). Expired-at-import and no post-expiry threshold are eligibility reasons, not delivery failures. `GET /api/tokens/:id/alert-state`, `GET /api/tokens/:id/alert-timeline`, and `GET /api/v1/workspaces/:id/control-center/alert-eligibility-summary`.
- **Azure Key Vault and Entra inventory scans accept an Entra app** (`authMethod: client_credentials`, tenant ID, client ID, client secret) in addition to a pasted access token. TokenTimer mints `vault.azure.net/.default` or `graph.microsoft.com/.default` from `login.microsoftonline.com/{tenant}/oauth2/v2.0/token`. Commercial Azure only; national clouds are not supported. Client-credential Entra scans attribute inventory to the tenant GUID from OpenID discovery. Key Vault mints against the supplied tenant and does not call OpenID discovery. A discovery rate-limit or Microsoft outage is reported as such, not as a bad tenant. Key Vault inventory lists metadata only, so Key Vault Reader is enough. Graph inventory recommends Application.Read.All. This is not CertOps Azure DNS. **Operator guidance:** existing pasted-token configs keep working.
- Vault inventory import supports AppRole (role ID / secret ID, optional custom `authMount`, optional Vault namespace) exclusive with a static token. Client tokens refresh at 80% of `auth.lease_duration`. A downstream 403 is not retried as expiry. A failed AppRole refresh aborts the scan. A mount list that fails after login is HTTP 403 `VAULT_PERMISSION_DENIED`, not 502. Recurring use needs a reusable SecretID. Address must be `http` or `https`. For a private Vault CA, set `NODE_EXTRA_CA_CERTS` on the API. See `docs/CONFIGURATION.md` (Vault AppRole authentication).
...
- Failed AppRole login responses log status and path only, never the Vault body. Credential fields (`token`, `roleId`, `secretId`, passwords, `Authorization`) are redacted from dashboard API request logs on localhost/dev/staging.
...
- Webhook test and delivery refuse HTTP 3xx redirects. The client resolves the destination itself, blocks private/reserved answers, and connects (directly or through `HTTP_PROXY`/`HTTPS_PROXY`) to those IPs. TLS SNI and the Host header still use the original hostname. **Operator guidance:** if private-IP enforcement is on (production default) and local DNS fails, delivery is blocked instead of letting the proxy resolve the name. Trust the proxy to filter destinations only when `WEBHOOK_ALLOW_PRIVATE_IPS=true`. Worker delivery honors proxy env without `NODE_USE_ENV_PROXY`; the Test button needs `NODE_USE_ENV_PROXY=1`. `NO_PROXY` accepts `*.example.com` and `host:port`.
- The Twilio WhatsApp status webhook uses its own limiter (`TWILIO_WEBHOOK_RATE_LIMIT_WINDOW_MS`, default 60s; `TWILIO_WEBHOOK_RATE_LIMIT_MAX`, default 1200 in production) instead of the plan API limiter. This is inbound StatusCallback only; product send does not set StatusCallback. **Operator guidance:** if provider callbacks return 429 `TWILIO_WEBHOOK_RATE_LIMITED`, raise `TWILIO_WEBHOOK_RATE_LIMIT_MAX`. Helm `twilio.webhookRateLimit*` still emit when `twilio.existingSecret` is set; put the limiter keys in that secret only if you prefer them there.
...
- Import filter regex rules reject overlapping quantified alternatives such as `(a|aa)+`. Remaining patterns must finish a 50ms match probe at save. Each match also runs under that 50ms budget; a timeout or unsafe pattern fails closed.
- CSRF double-submit checks run on every mutating `/api` and `/auth` route except logout, worker-to-API calls, and CertOps machine-token paths. Login, register, password reset, and 2FA verify now require `X-CSRF-Token`. CSRF is on in development as well as production; `NODE_ENV=test` still skips it. **Operator action:** cookie-session clients must `GET /api/csrf-token` and send the JSON `csrfToken` as `X-CSRF-Token`. Do not copy the cookie (`x-csrf-token` or `__Host-psifi.x-csrf-token`); it is an httpOnly HMAC, not the token. Missing header is 403 `EBADCSRFTOKEN`.
- `js-yaml` is pinned to 4.3.2, clearing [GHSA-2883-xcg3-v3hh](https://github.com/advisories/GHSA-2883-xcg3-v3hh). `nodemailer` is pinned to 9.1.1, clearing [GHSA-2x7j-588g-ccc2](https://github.com/advisories/GHSA-2x7j-588g-ccc2), [GHSA-8m3c-c648-2xjj](https://github.com/advisories/GHSA-8m3c-c648-2xjj), [GHSA-cc9r-2j5m-2m83](https://github.com/advisories/GHSA-cc9r-2j5m-2m83), and [GHSA-wmmp-3585-3rmp](https://github.com/advisories/GHSA-wmmp-3585-3rmp). `sharp` is pinned to 0.35.4, clearing [GHSA-rgj7-g3m4-5g8c](https://github.com/advisories/GHSA-rgj7-g3m4-5g8c). **Operator action:** rebuild the API, worker, dashboard, and k8s-controller images.
...
- The CertOps agent fleet table has a new Compatibility column showing each agent's Compatible/Outdated/Blocked state. Compatible means the agent meets this control plane; Outdated means it can still claim jobs but is more than one minor behind the latest known build; Blocked means it is below the accepted minimum and cannot claim any job until upgraded. Dummy reject-ceilings such as `99.999.999` are not shown.
...
- The CertOps jobs list now shows when a job needs manual reconciliation instead of only on the Audit page: a job stuck as `orphaned_unknown_effect` gets a red "Needs reconciliation" badge (previously an unlabeled gray badge), and its row shows either the recorded error or, if none was recorded, an advisory line explaining the side effects could not be confirmed. Pending and claimed jobs also say why they are waiting (assigned agent ineligible, offline, or still waiting to be claimed) instead of a bare status.
- The jobs panel and evidence timeline now show who approved or rejected a certificate job and when, using the approver's display name, instead of only the Audit page.
- **Workspace setting to always require approval before a certificate job runs.** Admins can turn on `certOpsRequireApprovalAlways` in a workspace's CertOps Settings tab (`GET`/`PUT /api/v1/workspaces/:id/certops/settings`) so every new job starts `pending_approval`, including jobs created from the dashboard, machine API tokens, bulk renew, scheduled renewal, and trust-anchor distribute or revoke. The per-job approval checkbox cannot override it. Off by default. The dashboard's manual job-creation modal shows and locks the approval checkbox as checked while the setting is on. Turning it on or off is audited (`CERTOPS_WORKSPACE_APPROVAL_POLICY_ENABLED`/`_DISABLED`), same as the existing pause/resume kill switch. A request that sets both this and the pause flag together applies them as one transaction, so a failure partway through leaves neither setting changed.
- Certificates due for renewal but waiting on their issuing CA's per-CA capacity limit now show a distinct "At CA capacity" badge and a separate, non-alarming notice in the Upcoming renewals panel, instead of looking indistinguishable from a healthy auto-renewing certificate. The capacity wait itself clears automatically once earlier renewals for that CA finish.
- **GCP scans now discover certificates alongside Secret Manager secrets.** Secrets and Certificates checkboxes on the GCP import form cover Google Cloud's Certificate Manager (global-location certificates) and Compute Engine `sslCertificates`, both global and regional load-balancer certificates. Requires granting `roles/certificatemanager.viewer` and `roles/compute.viewer` on the project, in addition to the existing `roles/secretmanager.viewer`, to the identity that minted the scan token. Obsolete-item cleanup on import covers the new certificate kinds the same way it already covers GCP secrets: a certificate kind hit by an API error, cut short by the scan's shared item budget, or missing one or more regions the Compute Engine API could not reach for this scan, is reported incomplete, never complete-and-empty, so cleanup skips it instead of deleting real certificates. A Compute Engine certificate with the same name in two different scopes (e.g. `global` and a region) is tracked as two distinct items rather than colliding into one. A failed secrets or certificate sub-scan now names the real Google reason (API not enabled, missing permission, or the token belonging to a different identity than the service account you granted) instead of axios's `Request failed with status code 403`.
- The "Deploy an agent" wizard now shows an in-product hint if the registration wait runs past the usual window, naming the likely causes (unreachable API URL, blocked outbound connection, an expired or reused bootstrap token, a mismatched CA bundle) and the per-OS command to check the agent's own logs on the target host.

### Fixed

- **Retiring a certificate as revoked or decommissioned now stops expiry and renewal-failure alerts for that certificate.** Pending renewal-failure alerts for it are dropped immediately; pending expiry alerts are dropped once no live sibling remains on the shared token. New ones are not queued, and weekly digests omit a token whose lifecycle is retired. Endpoint-down alerts are unchanged.
...
- **`distribute-trust` crashed instead of returning a tagged failure on a read-only or unwritable Linux anchors directory.** An unguarded filesystem write let an EROFS/EACCES error escape as a generic, uncategorized failure, indistinguishable from any other crash. Now returns a tagged `trust_store_not_writable` failure, matching the handling `revoke-trust` already had for the same case.
- **AlmaLinux 9 (and other RHEL-family hosts) could report a genuinely successful install as failed, then strand that anchor permanently.** `update-ca-trust extract`'s underlying `p11-kit` step calls `chmod` on the extracted directory-hash bundle, an operation that needs ownership no POSIX ACL grant can confer, so `extract` can exit non-zero even while the consolidated trust bundle is correctly regenerated and the certificate is genuinely trusted. `distribute-trust` reported that as `os_mutation_failed` regardless, and the resulting `pending_install` receipt then made every later `revoke-trust` on the same target refuse with `receipt_pending_install` - permanently, since re-running `distribute-trust` hit the identical false negative again. `distribute-trust` now re-observes the real state of the update command's own output directory (the consolidated bundle it deposits, not the input anchor file the agent itself already wrote) before trusting a non-zero exit code, reporting `installed` (with the command's output carried as a non-fatal warning) only when the certificate is genuinely present there. `revoke-trust` can additionally unwind a `pending_install` receipt already stranded by this bug on an older agent, as long as the anchor is still observably present; the refusal is unchanged when it genuinely is not.
- **Revoking a trust anchor on Debian/Ubuntu could leave dangling `/etc/ssl/certs` symlinks.** Plain `update-ca-certificates` reports "0 added, 0 removed" and does not prune a just-removed anchor's symlink; only `update-ca-certificates --fresh` does, and no amount of retrying the previously-allowlisted plain command fixed it. `revoke-trust` now prefers a policy-supplied `--fresh` argv under a new, opt-in `trust-store:update-ca-certificates-fresh` profile when the operator has configured it, and otherwise falls back to the previous behaviour with a non-fatal `debian_fresh_command_ref_missing` warning on the result rather than silently leaving the symlink. **Operator action:** add the `trust-store:update-ca-certificates-fresh` profile (see `docs/certops/agent.md`) to self-heal existing dangling symlinks on future revokes; without it, revoke behaves exactly as before.
- Corrected the 0.14.1 changelog entry and `docs/certops/agent.md`, which both overstated the Linux trust-store grant: AlmaLinux 9 was claimed to be fully covered by the automatic ACL grant, when the grant is necessary but not sufficient (see above). See `docs/certops/agent.md` for the corrected AlmaLinux 9 and Debian-revoke guidance, and the new `certops-trust-store-linux` runbook for full operator-facing detail.

### Added

- `docs/certops/CONTEXT.md` documents the previously-undocumented `GET /api/v1/workspaces/:id/certops/trust-anchors/:anchorId/installations` endpoint.
- `docs/certops/agent.md` documents `PrivateTmp=true` in the shipped systemd unit: without it, `ProtectSystem=strict` leaves `/tmp` read-only, and `update-ca-certificates`/`update-ca-trust extract` both fail at `mktemp` before ever touching the anchors directory - easy to mistake for the anchors-directory grant itself being missing.

## [0.14.1] - 2026-09-01

### Added

- **Trust-anchor management in the dashboard.** The CertOps Agents tab now has a Trust anchors panel: approve a CA certificate as a workspace trust anchor, list existing anchors with type, fingerprint and status, expand one to see every agent it has been distributed to and that installation's current state, and drive distribute or revoke from there. Jobs, audit, and the evidence timeline name the agent by hostname. Refresh sits beside Distribute and updates the rows in place. Retiring an anchor is confirmed in-place and states plainly that it stops new distributions without removing anything already installed. Previously trust anchors could only be managed through the API. Workspace-admin only, matching the `certops.trust_anchor.manage` permission.
- **`GET /api/v1/workspaces/:id/certops/trust-anchors/:anchorId/installations`** returns every installation row for a trust anchor (agent, store, transition state, provenance and last error). Read-only and workspace-admin gated. It deliberately still answers for a workspace whose certificate operations are paused, so an operator can inspect where an anchor landed while the workspace is halted.
- **Obsolete-item cleanup on import now covers Vault, AWS, Azure Key Vault, Azure AD and GCP Secret Manager**, not just GitHub and GitLab, and the auto-sync worker uses the same cleanup path for every provider via a per-config `cleanup_obsolete` flag. Cleanup scope is derived from a server-side scan record rather than from values the browser sends back, and only runs against sub-scopes the scan reported fully complete, so a truncated or partly-errored scan cannot delete tokens it never actually checked. Tokens carry structured provenance (provider, instance, owner, kind), so two instances of the same provider cannot delete each other's tokens. Each scan can drive at most one cleanup, and an older scan's cleanup cannot delete a token a newer concurrent scan just rediscovered. AWS All Regions + Global is one scan with a single `scan_id`. GitLab cleanup-on scans force include-revoked off so a just-revoked token is not kept. A GCP Secret Manager version-lookup miss still leaves the secrets kind complete (the secret was already listed). Azure AD FOUND counts are extracted secrets/certs, not Graph object counts. Vault cleanup copy is limited to the kinds that scan actually ran. **Operator guidance:** tokens imported before this release are left untouched by cleanup where their originating instance and owner cannot be established from the old location string; a later scan-bound import of the same name+location adopts that row instead of inserting a duplicate. New migration (`50`) adds the scan tables and the token provenance columns.

### Fixed

- **A bulk-renew request without an explicit idempotency key permanently blocked any later renewal of the same certificate.** Each item synthesized a key derived only from the certificate id, so the second renewal of a certificate always collided with the first. Bulk renew now leaves the key unset when the caller supplies none.
- **`dryRun` on bulk renew reported certificates as renewable without resolving their renewal profile**, so a certificate that would fail on a real call could pass the preview. The dry-run path now runs the same read-only preflight the real call does.
- The maintenance worker now exits non-zero when a CertOps sweep fails, instead of reporting success and letting a scheduler treat a failed maintenance pass as healthy.
- A trust-anchor job whose lease expired left its installation row stranded mid-transition unless the optional reconciliation sweep was enabled. The lease reaper now unwinds the transition itself, so the row is corrected even with that sweep switched off.
...
- Linux trust-anchor distribute failed with `EROFS` under the hardened unit: `ProtectSystem=strict` left `/usr/local/share/ca-certificates` and `/etc/ssl/certs` (Debian/Ubuntu) or the RHEL pki anchors read-only, and `tokentimer-agent` had no write ACL there. The agent still advertised `trust-anchor-deploy-v1` because the update command existed. **Operator action:** `install-agent.sh` now opens those directories automatically on Ubuntu 22.04/24.04/26.04 and AlmaLinux 9 (setfacl, or group-write with owner left root when the `acl` package is missing). Pass `--no-trust-store` to skip. Do not pass the CA directories as `--write-path`. Windows Server 2019/2022/2025 needs no extra grant (LocalSystem already has `LocalMachine\Root` / `LocalMachine\CA`); `install-agent.ps1` accepts `--trust-store` so a Linux command pasted into PowerShell does not fail. On an already-installed Linux agent, re-run the installer or add the family paths to the unit drop-in, grant write as above, then `systemctl daemon-reload && systemctl restart tokentimer-agent`. **Correction (0.14.2):** on AlmaLinux 9 this ACL grant is necessary but not sufficient - `update-ca-trust extract`'s `p11-kit` step can still exit non-zero even when the CA is genuinely trusted, which this release reported as a false-negative failure. See the 0.14.2 entry above.

## [0.14.0] - 2026-08-30

### Added

- **Machine trust-store distribution for CA trust anchors.** Operators can approve a CA certificate (root or intermediate) as a workspace trust anchor and distribute it to, or remove it from, a specific agent's machine trust store. Windows (`LocalMachine\Root` / `LocalMachine\CA`), Debian/Ubuntu (`update-ca-certificates`) and RHEL/Fedora (`update-ca-trust`) are supported. An agent advertises the `trust-anchor-deploy-v1` capability only where the required trust-store directory and update command actually resolve, and the control plane will not dispatch a trust job to an agent whose capability declaration is missing or stale.
  - Trust anchors are approved material: the certificate must parse and be a CA, and its SHA-256 fingerprint is pinned on the anchor record and every job referencing it. The agent re-verifies the fingerprint against the PEM before touching a trust store, and the destination store is derived from the approved anchor type rather than the certificate's own contents.
  - Distribution is reference-counted per (agent, store, fingerprint, owner) and records whether TokenTimer installed the material or found it already present, so removing a trust anchor never deletes a certificate TokenTimer did not install. Removing material that is already absent reports success. Already-present material is detected against the real trust store rather than a filename convention, so a CA an operator installed independently is reported as already-present instead of being duplicated.
  - Each transition requires an idempotency key and carries a generation number, so a replayed request cannot double-apply and a superseded transition's late result is rejected rather than applied out of order. The agent keeps a local ownership receipt, so an install interrupted between writing that receipt and completing the OS mutation is recoverable: the next attempt re-probes the OS store and reclaims the stale receipt when the fingerprint is genuinely absent, rather than refusing that (store, fingerprint) forever.
  - Retiring an anchor stops new distributions but never silently fans out removals; existing installations are removed only by an explicitly authorized removal. A worker reconciliation sweep reschedules transitions still in flight and reports ones that stop making progress instead of retrying them forever.
  - New endpoints: `GET`/`POST /api/v1/workspaces/:id/certops/trust-anchors` and `POST /api/v1/workspaces/:id/certops/trust-anchors/:anchorId/retire`, gated behind the new `certops.trust_anchor.manage` permission (workspace admin only). A malformed `agentId` is rejected with `CERTOPS_TARGET_AGENT_INVALID` (400) and an unregistered or cross-workspace one with `CERTOPS_TARGET_AGENT_NOT_FOUND` (404), before any job is created. Idempotency keys are scoped to the operation and full target tuple, so reusing a key against a different operation or target is rejected with `CERTOPS_TRUST_JOB_IDEMPOTENCY_CONFLICT` (409) rather than replaying the wrong job.
  - The agent-local `allowedCommands` policy gate covers trust work on every platform, including Windows under the `trust-store:certutil` profile name: a renew-only agent with no profiles configured refuses trust-anchor jobs rather than running them unconditionally.
  - New database migrations add the `certops_trust_anchors` and `certops_trust_anchor_installations` tables, installation-tracking columns, and a reference-release idempotency ledger. **Requires PostgreSQL 15+** (see Configuration).
  - Releasing a reference that is not the last one marks only that owner's row `removed` and creates no job, so one owner's `revoke-trust` never removes material another owner still depends on. Whether material is physically removed is decided from the tuple's full provenance history, and a reference that was only ever `preexisting` is released without dispatching a removal.
  - **Operator guidance:** a trust job that never leaves `pending` is usually waiting on the target agent to advertise `trust-anchor-deploy-v1`; it waits rather than failing loudly. Revoking one owner's reference does not remove the certificate while another owner still references the same anchor on the same agent; removal happens once the last live reference is released. A `receipt_finalize_conflict` means the OS mutation did succeed and the conflict is recorded in `last_error`; see `docs/certops/agent.md` section 9 for the recovery path.

### Changed

- **BREAKING: manual and bulk certificate renewal requests no longer accept an arbitrary execution-field payload override.** The renew and bulk-renew routes now resolve every execution field (`commandRef`, `caEndpoint`, `target`, `sans`, `keyAlgorithm`, `dnsProvider` and the rest) from the certificate's stored renewal profile, exactly as the scheduler does. Only `reason` may still be overridden; any other field is rejected with `CERTOPS_RENEWAL_OVERRIDE_INVALID` (400) before any certificate is touched, instead of being silently applied or dropped. **Operator action:** any integration passing execution fields on a manual or bulk renew call must remove them and rely on the certificate's renewal profile, or the call will now fail.
- **`CERTOPS_AGENT_REQUIRE_SIGNED_AGENT_ID` now defaults to `true`.** A signed job missing `agentId` entirely is now rejected with a distinct incompatibility error instead of being silently tolerated. A job whose `agentId` is present but does not match the agent's own identity was already rejected unconditionally in every prior release and is unaffected. The `agent-id-binding-v1` capability is advertised automatically while the flag's effective value is `true`, and stops being advertised on the next heartbeat after it is overridden back to `false`. **Operator action:** set the flag (or `requireSignedAgentId` in `config.json`) back to `false` only as a temporary rollback, while this agent still talks to a control plane that has not finished emitting `agentId` on every signed dispatch; remove the override once that control plane is upgraded. The flag is planned for removal in a future release.
...
- New evidence event types `trust.distributed` and `trust.revoked`, and new audit events `CERTOPS_TRUST_ANCHOR_DISTRIBUTED`/`CERTOPS_TRUST_ANCHOR_REVOKED`, fired on a trust-anchor job's successful terminal transition. A terminal failure still uses the generic `CERTOPS_JOB_FAILED` event, carrying the same trust-result fields so a failed job's audit trail is no less informative. Trust-anchor audit actions carry the anchor's name, fingerprint and host alongside the IDs, and the dashboard's Audit page has a dedicated formatter and filters for them.
- The dashboard's manual job-creation modal no longer offers the execution-override fields for a `renew` operation, matching the renewal-override restriction above.
- New worker sweep, disableable independently of every other sweep: `CERTOPS_SWEEP_TRUST_ANCHOR_RECONCILIATION_ENABLED` (default on) and `CERTOPS_SWEEP_TRUST_ANCHOR_RECONCILIATION_TIMEOUT_MS`, reported on the new `certops_trust_anchor_reconciliation_installations` Prometheus gauge.

### Fixed

- The CertOps post-migration integrity check (`certops:integrity-check`) failed its capability-audit assertion for every agent that registered and never subsequently changed its declared capabilities. Registration audits as `CERTOPS_AGENT_REGISTERED`, but the assertion only accepted `CERTOPS_AGENT_CAPABILITIES_CHANGED`, so a healthy agent counted as missing its audit trail. It now accepts either action.
- Agent log lines had no timestamp, and the outbox-transmission-failure and lease-renew-failure lines carried a doubled `tokentimer-agent:` prefix. Every line now starts with an ISO-8601 UTC timestamp, the prefix is added once by the shared logger, and the job-finished line includes `errorMessage`/`failureCategory` when the job failed.
- `certificate_jobs.result_metadata` was left at its insert-time default (`{}`) for every deploy job, and no `certificate_job_log` row was written when an agent reported a real result. Agent-result ingestion now persists the reported result fields (`rejectionReason`/`keyRotated`, plus the full trust-result contract for trust-anchor jobs) and inserts a matching `certificate_job_log` row.
