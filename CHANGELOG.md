...
- **TokenTimer Agent** (`packages/agent`) — an outbound-only execution-plane process that registers against the control plane, polls for signed work, enforces agent-local policy, and reports schema-valid results and evidence:
  ...
  - Native DNS-01 solvers for eleven providers (Cloudflare, Route53, Google Cloud DNS, Azure DNS, RFC 2136, acme-dns, OVHcloud, Hetzner Console/Cloud DNS, Infomaniak, Exoscale, PowerDNS) plus a guided onboarding surface. DigitalOcean DNS is not yet supported.
...
Live end-to-end testing of the CertOps agent path, ahead of first release, found several defects in the paths above that shared one shape: work that succeeded, was never connected to the system meant to manage it, and reported nothing wrong. All were fixed before this feature ever shipped, and are recorded here because they explain design decisions elsewhere in this entry.

- **Nothing TokenTimer issued would ever renew itself** - the renewal scheduler only dispatches a renewal it can fully specify, and that specification lives in a `certificate_profiles` row linked from the certificate. Nothing ever wrote one: no route, no code path setting `managed_certificates.profile_id`, no derivation. Every certificate was counted as an incomplete-profile skip on every sweep, so automatic renewal could not fire for anything issued through TokenTimer. An operator saw a healthy `active` certificate with a real expiry, no failed jobs, and no indication it would silently expire. The renewal profile is now derived from the issue job payload that just succeeded and persisted in the same transaction that promotes the certificate, named `Derived: <common name> (<certificateId>)` and editable within a safe subset (see the renewal automation control surface, above). The name embeds the certificate id, not just the common name, so two certificates that happen to share a CN (load-balanced pairs, blue/green, repeated internal hostnames) always derive distinct profiles rather than one shared profile whose auto-renew switch and lead time would otherwise apply to both from either certificate's control; see ADR-0010 Amendment 2. Required payload fields (`caEndpoint`, `commandRef`, `dnsProvider`, `dnsZone`, `certPath`) are mapped rather than defaulted, SANs are pinned to what the CA actually issued, and a derivation that cannot complete names the missing field in the API log without failing the issuance. Per-certificate renewal lead time is the profile's `renew_before_days`, overriding the fleet-wide `CERTOPS_RENEWAL_THRESHOLD_DAYS`. See `docs/adr/0010-certops-derived-renewal-profiles.md`.
- **A renewal failure could notify nobody, and a failed issuance never did** - two independent causes with the same symptom. First, the rule for which jobs alert had two homes that disagreed: the dispatcher's set said `{renew, issue}` while the alert resolver rejected anything but `renew`, so failed issuances were handed to a path that discarded them. It now lives in one dependency-free module imported by both sides. Second, contact resolution and the alert insert ran inline under a savepoint so they could never abort result ingestion, which meant any error lost the intent to alert to a single log line with no retry and no record. The new `certops_outbox` table (migration 35) records the intent inside the transaction that decides the job failed, and a new `outbox-drain` worker sweep resolves contacts and queues the alert with backoff under an owner-scoped lease. A failed **issuance** deliberately does not raise `cert_renewal_failed`: nothing exists yet that is at risk of expiring, and there is no linked token to route contacts through, so issuance problems are visible as `provisioning` certificates instead. Classification is by transition origin, not status alone, so an agent's policy refusal alerts while an operator rejecting their own job does not. See `docs/adr/0009-certops-durable-side-effects-and-alert-policy.md`.
- **A certificate could be promoted to `active` on evidence that did not prove it exists** - reconciliation used the newest `validation.passed` evidence for the job. The agent emits that twice per run (once when ACME returns, once after reading the deployed file back), evidence outlived the attempt that produced it, and promotion required neither a fingerprint nor an expiry. So a row could go `active` describing the certificate that was requested rather than the one on disk, carry a previous attempt's fingerprint, or have no expiry at all, which looks healthy and is silently unmanaged. Promotion now requires `verify`-step evidence bound to the job's current claim (`certificate_evidence` gains `claim_id` and `attempt_count`, migration 36) carrying both a fingerprint and a parseable expiry. A certificate that fails those checks stays `provisioning` with an operator-visible `reconciliation_reason` (`no_claim_bound_verify_evidence`, `verify_evidence_missing_fingerprint`, `verify_evidence_missing_expiry`). Because reconciliation now depends on claim-bound evidence, `issue` jobs and `renew` jobs against a still-`provisioning` certificate are only offered to agents declaring the `evidence-claim-binding-v1` capability; an out-of-date agent simply is not offered the work, which shows up as an unclaimed `pending` job. Agents without the capability keep claiming ordinary renewals of `active` certificates unchanged. Token linking also moves to successful reconciliation, since a certificate that does not exist yet has no expiry to mirror onto a token.
- **Concurrent issuance retries with the same idempotency key could return HTTP 500** - the workspace kill-switch lock does not serialize two issuance requests against each other, so both could read "no existing certificate" and both attempt the insert. One won; the other died on a unique index and surfaced as an opaque 500 when the honest answer was the job the first request had already created. Identity resolution is now serialized on a per-identity advisory lock, leaving unrelated issuance in the same workspace parallel. A `payload.certPath` that is relative or a directory is now rejected at request time as well: both shapes previously failed agent-side only after the ACME order had been placed, burning a rate-limited order and stranding a `provisioning` row.
- **One certificate file could acquire two inventory identities** - an agent that issues to a path later scans that path and reports it as an ordinary filesystem discovery, and the two sources are keyed differently, so a duplicate row appears. Issuance now records the deployed path and owning agent on reconciliation (`deployed_cert_path`, `deployed_agent_id`, migration 36), which is what a correlation on `(agent, path)` needs, and a partial unique index reserves that pair. The discovery ingest does **not** yet consult it: its upsert still keys on `(workspace_id, source, source_ref)`, so a later `agent_filesystem` scan of an `agent_issuance` path still creates a second row. Recording the pair is the prerequisite, not the fix; the correlating lookup is still open.
- **A fleet that renewed nothing looked identical to an idle one** - the renewal scheduler exported only its `created` count. All outcomes are now series on `certops_renewal_scheduler_certificates` labelled by `outcome`: `scanned`, `created`, `replayed`, `skipped_paused`, `skipped_ca_cap`, `skipped_incomplete_profile`, `skipped_not_agent_deployable`, `skipped_auto_renew_disabled`, `errors`. An observed-only certificate (no agent custody of its key, so it can never be agent-renewed) counts as its own skip rather than an error that would make every sweep look broken, and a certificate whose renewal was switched off deliberately is likewise distinguished from one being refused.
- **A renewal deployed the leaf certificate only, so servers served an incomplete chain** - both ACME adapters pinned a chain and a fullchain output path but returned neither, and the deploy step read the leaf. Clients that do not fetch intermediates themselves saw an untrusted certificate on a certificate that every TokenTimer surface reported as healthy. `certPath` now receives the **fullchain** by default, falling back to the leaf only when the CA or tool produced no chain artifact, and a target that configures `chainPath` gets the leaf and the intermediates written to their separate destinations atomically. **This changes the bytes written to `certPath` for existing renewals**: a host that was silently serving an incomplete chain starts serving a complete one at its next renewal, and a host whose configuration already concatenated the chain elsewhere should be checked for duplication. The underlying reason this was a blocker rather than a cosmetic fix: both adapters resolved those output paths relative to the process working directory, which under systemd sandboxing is a read-only `/`, so the run aborted *after* the ACME order had already succeeded, burning a rate-limited order and stranding the certificate. All artifacts are now written in a job-scoped directory next to the destination, and every one of them is removed on both the success and the failure path.
- **Retired certificates were still counted as live inventory** - `cert_lifecycle_status` was written on retire and never read back, so revoked and decommissioned certificates kept inflating Control Center's total assets, expiry buckets, needs-attention count, never-expires list and source breakdown. One shared exclusion predicate now applies to every one of those queries, with a guard test that fails if a future query is added without it. **Operators will see asset counts drop** in any workspace holding retired certificates; the numbers were wrong before, not now. Retired assets stay reachable from the main dashboard on request, and no separate "retired" stat was added, since a second number invites the two from disagreeing.
- **The certificate operations panel contradicted its own description** - it claimed to list "managed certificates linked to your token inventory" while listing every managed certificate, including ones with no token, mixing rows with no expiry into an expiry-sorted list. It now lists token-linked certificates and reports `provisioning` and unlinked-but-active certificates as separate counts rather than dropping them, so no asset can be silently absent from an inventory panel.
- **A certificate TokenTimer issued and renews by itself had a one-row audit trail** - searching the audit log for such a certificate returned `CERTOPS_JOB_CREATED_MANUAL` and nothing else. In between, the system had ordered from a public CA, written a key and a certificate to a host, reloaded a service, promoted the certificate to `active`, and granted itself a profile authorising it to repeat that indefinitely; a scheduled renewal produced no row at all. The gap survived because until profile derivation (above) none of those paths could actually run, and they all went live at once. Five machine-initiated events close it: `CERTOPS_CERTIFICATE_ISSUED` on promotion, `CERTOPS_CERTIFICATE_ISSUANCE_UNRECONCILED` when a succeeded job leaves a certificate unusable, `CERTOPS_RENEWAL_PROFILE_DERIVED` when TokenTimer grants itself renewal authority, `CERTOPS_JOB_CREATED_AUTOMATIC` as the long-implied counterpart to the manual event, and `CERTOPS_AGENT_REGISTERED` for the enrollment that turns a machine into a principal with command rights (also the only record of a bootstrap token being spent, which the audit catalog already told readers to look for). Each is written on the same transaction as the state change it describes, so a certificate cannot become active, and a profile cannot come into existence, unaudited. None carries a user actor, since none has one: the acting agent is named in metadata. Job successes are deliberately not audited individually and neither are replays or no-ops. `CERTOPS_RENEWAL_PROFILE_UPDATED`, which a runbook already referenced, is now documented too. See `docs/adr/0011-certops-machine-initiated-audit-events.md`.
- **The agent declared claim-bound evidence but had no way to send it** - the agent registers the `evidence-claim-binding-v1` capability, which is what makes the control plane offer it `issue` jobs and renewals of a still-`provisioning` certificate. But `buildEvidenceBody` and the protocol client's `reportEvidence` only ever constructed `{ jobId, evidenceItems }`; there was no parameter through which a `claimId` could reach the wire, so the capability was advertised but structurally unusable. The evidence builder and protocol client now accept and thread an optional `claimId`/`attemptId`, and `executeJob` binds every `reportEvidence` call made during a job's execution to that job's current claim before any evidence leaves the process, so the capability now describes what the agent actually does.
- **An `issue` job could be created missing the fields it needs to run** - `validateExecutionFields` validated execution fields when present but enforced no required set, so an `issue` request could omit `commandRef`, `caEndpoint`, `certPath`, `dnsZone` or `dnsProvider` and still create a job (and, for issuance specifically, a persistent `provisioning` certificate row) that could only fail later, on the execution plane, potentially after a real ACME order had already been placed. These five fields are now required at job-creation time for `issue`, before any row exists.
- **An incomplete post-renewal evidence refresh looked identical to a healthy renewal** - reconciliation of a fresh issuance already refused to promote a certificate on evidence missing a fingerprint or expiry and recorded why. The equivalent step for renewing an already-`active` certificate did not: it silently returned, leaving `not_after` unchanged. Because the renewal scheduler's idempotency key is `(certificateId, notAfter)`, a `not_after` that never advances make the next sweep re-derive the same key and collide with the job that had just "succeeded," which could repeat indefinitely against the same CA. This path now mirrors the issuance path: it sets a `reconciliation_reason` on the certificate and records a `CERTOPS_CERTIFICATE_RENEWAL_UNRECONCILED` audit event instead of returning silently, and on success it mirrors the refreshed fingerprint, validity window and `not_after` to the certificate's linked token in the same transaction, which previously only happened on first reconciliation, not on every subsequent renewal.
- **Renewal adoption was unreachable for the certificates operators would expect to adopt** - "set up automatic renewal" for an existing certificate requires `managed_certificates.deployed_cert_path` (the destination a renewal would be deployed back to); refusing without it is correct, but `agent_filesystem` discovery, the source that populates the majority of an operator's inventory, never wrote that column, so every ordinary agent-discovered certificate failed the precondition regardless of eligibility. Filesystem discovery now records the observed path and owning agent alongside the rest of the observation, so a certificate an agent already watches can be adopted into automatic renewal without a redeploy first.
- **`reconciliationReason` was recorded but not visible outside the audit log** - a certificate stuck in `provisioning`, or one whose most recent renewal evidence was incomplete, carried `managed_certificates.reconciliation_reason`, but the field was absent from the certificate inventory API, its OpenAPI schema, and the certificates page, so "operator-visible" meant "visible if you know to search the audit log for this certificate's id." It is now part of `CertOpsManagedCertificate` and rendered as an "Unreconciled" badge next to the certificate's status.
- **`CertOpsJobDetail`'s status enum lagged `CertOpsJob`'s** - both describe the same `certificate_jobs.status` column and both are `additionalProperties: false`, but only the list schema's enum included the two newest terminal statuses, `dry_run_complete` and `orphaned_unknown_effect`; a strict client validating a job detail response against the documented enum would reject a real, valid terminal state. The two enums are now identical.
- **Force-retiring an agent ended its in-flight renewals without alerting anyone** - the H12 fencing sweep cancelled `claimed` jobs and marked `running` jobs `orphaned_unknown_effect` with raw `UPDATE`s and no outbox intent, so a renewal killed by retiring its agent produced a dead renewal that emailed nobody, indistinguishable from no renewal having failed. `forced_retirement` was already a defined transition origin with its own policy test; only the call was missing. Fencing now enqueues the renewal-alert intent on the retire transaction's own client (`enqueueOutboxEvent` refuses a non-transactional caller by construction), keyed on the job id like every other origin so a job the lease reaper already alerted for is not alerted for twice.
- **Executor-reported renewal failures bypassed alerting entirely** - a `renew` job failed through `POST /api/v1/certops/jobs/:jobId/events` (certctl, the Kubernetes controller) recorded the status and stopped, while the identical failure reported by an agent notified the operator. Executor event ingestion now classifies the terminal transition through the same policy and records the same intent, under a new `executor_event` origin so the alert says which lane ended the job.
- **An executor machine token could terminate a job an agent was actively running** - the executor and agent lanes share `certificate_jobs` but only the agent lane holds a lease, and nothing stopped an executor `job.completed`/`job.failed`/`job.rejected` from moving a job out from under a running agent, which then reported its own result against an already-terminal row while its real side effects had landed. Terminal executor events against a job with a live `claimed_by_agent_id` are now refused with `409 CERTOPS_EXECUTOR_JOB_AGENT_CLAIMED`; non-terminal progress events stay accepted, since narrating is harmless.
- **A staged private key could outlive its job** - every *returned* renewal failure discarded the staged key, but a *thrown* one skipped it: the `finally` block cleaned the CSR and the staged chain, never the key. Worse, `discardStagedKey` itself threw on any non-`ENOENT` unlink error at five unwrapped call sites, converting a clean `failed` into an exception that escaped before the rest of the cleanup ran and orphaning the very key it was asked to remove. The discard is now best-effort and reports its outcome, both throwing paths (ACME block and deploy tail) discard before re-raising, and an unremovable staging file produces a warning naming the path instead of silence.
- **A multi-target issuance derived a single-target renewal profile** - profile derivation reads `deploymentTargets[0]` and nothing else, so an `issue` payload carrying several targets deployed to all of them and then produced a renewal profile describing one: the remaining hosts silently stopped being maintained at the first renewal, months later. `issue` jobs now refuse more than one `deploymentTargets` entry at request time, and derivation declines rather than truncating if such a payload reaches it anyway. `renew` is deliberately unconstrained, because its multi-target support comes from an explicit profile rather than positional inference.
- **A declined profile derivation left no reason anywhere** - when derivation refused (incomplete payload, operator-owned name collision, link conflict) the only trace was `profileId: null` buried in `CERTOPS_CERTIFICATE_ISSUED` metadata, and the log line intended for exactly this case was unreachable because the issuance caller passed no logger. The resulting state is the worst kind of quiet: certificate active, expiry real, job succeeded, will never renew. Declines now set `managed_certificates.reconciliation_reason` (surfaced by the "Unreconciled" badge above), write a `CERTOPS_RENEWAL_PROFILE_DERIVATION_DECLINED` audit event naming the missing field but never its value, and carry the cause forward as `profileDerivationReason` on the ISSUED event.
- **A rejected `issue` job with a missing required field returned 500 instead of 400** - the requiredness gate on `commandRef`/`caEndpoint`/`certPath`/`dnsZone`/`dnsProvider` throws `CERTOPS_JOB_EXECUTION_FIELD_REQUIRED`, but the route layer's error map was never taught that code, so a well-formed rejection of bad client input fell through to a generic `500 INTERNAL_ERROR`. Added to the route's bad-request mapping alongside its sibling `CERTOPS_JOB_EXECUTION_FIELD_INVALID`.
- **The auto-renewal off switch was enforced in exactly one place, not two** - `disabled`/`archived` profiles are excluded by the renewal scheduler's own scan query, but nothing re-checked `certificate_profiles.status` at job creation itself, so any future caller of `createCertificateJob` that reached a disabled-profile certificate by a path other than that one scan (an alternate scheduler entry, a batch job) would have renewed it anyway - the same "one query away from the guarantee" shape as several fixes above. `resolveManagedCertificateJobDefaults` now re-checks the linked profile's status and refuses (`409 CERTOPS_RENEWAL_AUTO_RENEW_DISABLED`) whenever an automation-sourced `renew` job targets a disabled/archived profile, closing the gap at the function that actually creates the job rather than only at the one caller that currently reaches it. Scoped to `source: "automation"` only: a manager manually renewing, or bulk-renewing, a certificate whose profile has auto-renew switched off remains the documented way to still renew it yourself - only the scheduler's automatic pickup is supposed to skip it.
...
- **Windows is not a supported deployment target for the control plane or the CertOps agent.** The agent's own test suite runs on Windows and several code paths degrade gracefully there (best-effort POSIX file modes, skipped directory fsync, Windows-form absolute-path detection — see `docs/certops/agent.md` §8), but this is defense-in-depth for contributors developing on Windows, not a supported production posture. Production agent hosts and the control plane itself target Linux with systemd.
- **DigitalOcean DNS is not among the eleven supported DNS-01 providers** in this release.
