...
- **`distributeTrust` threw instead of returning a tagged failure on a read-only or unwritable Linux anchors directory.** `installLinuxAnchorFile`'s `mkdirSync`/`writeFileSync` were unguarded, so an EROFS/EACCES escaped all the way out of `executeTrustJob` as a generic `errorMessage: "job execution failed: ..."` with no `failureCategory`, indistinguishable from any other crash. Now returns a tagged `trust_store_not_writable` failure, mirroring the guard `removeLinuxAnchorFile` already had.
- **AlmaLinux 9 (and other RHEL-family hosts) could report a genuinely successful install as failed, then strand that anchor permanently.** `update-ca-trust extract`'s underlying `p11-kit` step calls `chmod(2)` on the extracted directory-hash bundle, an operation that needs ownership no POSIX ACL grant can confer, so `extract` can exit non-zero even while the consolidated trust bundle is correctly regenerated and the certificate is genuinely trusted. `distributeTrust` reported that as `os_mutation_failed` regardless, and the resulting `pending_install` receipt then made every later `revoke-trust` on the same target refuse with `receipt_pending_install` - permanently, since re-running `distribute-trust` hit the identical false negative again. `distributeTrust` now re-observes the real state of the update command's own output directory (the consolidated bundle it deposits, not the input anchor file the agent itself already wrote) before trusting a non-zero exit code, reporting `installed` (with the command's output carried as a non-fatal warning) only when the certificate is genuinely present there. `revoke-trust` can additionally unwind a `pending_install` receipt already stranded by this bug on an older agent, as long as the anchor is still observably present; the refusal is unchanged when it genuinely is not.
