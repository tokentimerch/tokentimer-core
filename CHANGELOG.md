...
- **TokenTimer Agent** (`packages/agent`) — an outbound-only execution-plane process that registers against the control plane, polls for signed work, enforces agent-local policy, and reports schema-valid results and evidence:
  ...
  - Native DNS-01 solvers for twelve providers (Cloudflare, Route53, Google Cloud DNS, Azure DNS, RFC 2136, acme-dns, OVHcloud, Hetzner Console/Cloud DNS, Infomaniak, Exoscale, PowerDNS, and the test-only `pebble-challtestsrv` used to exercise the DNS-01 path against a local ACME test CA without a real DNS provider account) plus a guided onboarding surface. DigitalOcean DNS is not yet supported.
...
- **DigitalOcean DNS is not among the eleven production-grade supported DNS-01 providers** in this release (a twelfth, `pebble-challtestsrv`, exists only to exercise the DNS-01 path in tests against a local ACME CA and is not a real provider integration).
