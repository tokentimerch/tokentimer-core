...
- An asset (token, certificate, or CertOps agent) can belong to more than one contact group. Overlapping recipients get one live alert and one weekly digest for the unioned assets, not one message per group. Existing single-group assignments keep working. Multi-group writes are **on by default**; set `CONTACT_GROUP_PLURAL_WRITES=false` only as a mixed-fleet kill switch during a rolling upgrade. **Operator action:** every API and worker replica must already run the dual-write join-table release before this one; do not skip it. While any dual-write-only replica remains, keep `CONTACT_GROUP_PLURAL_WRITES=false` (Helm `api.env.CONTACT_GROUP_PLURAL_WRITES`) so a two-or-more `contact_group_ids` write is 400 and the dashboard stays single-select. **Operator guidance:** `contact_group_ids` is the new write/read field. Old `contact_group_id` still works and returns the lexicographically smallest assigned group (or null when the asset uses the workspace default). Generic digest webhooks keep `contact_group` and add `contact_groups`. Digest skip keys store HMAC-SHA256 of the destination (keyed by `WEEKLY_DIGEST_RECIPIENT_KEY`, else `SESSION_SECRET`), not the address or webhook URL. A failed digest recipient is not retried by the default Monday 09:00 job (that run is a new week). Rerun the digest the same week, or schedule it more than once weekly; `sent` claims make extra runs safe.
