...
- **GCP scans now discover certificates alongside Secret Manager secrets.** A "Scan certificates too" checkbox on the GCP import form covers Google Cloud's Certificate Manager (global-location certificates) and classic Compute Engine `sslCertificates` used by load balancers. Requires granting `roles/certificatemanager.viewer` and `roles/compute.viewer` on the project, in addition to the existing `roles/secretmanager.viewer`, to the identity that minted the scan token. Obsolete-item cleanup on import covers the new certificate kinds the same way it already covers GCP secrets.
