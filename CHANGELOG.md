...
- **Agent installer: re-running it to upgrade did not load the new code** — `install-agent.sh` stages the new package into `app.staging.$$` and swaps it in atomically, but then called `systemctl start`, which is a no-op when the unit is already active. The upgraded tree sat on disk while the old process kept running until the next manual restart or reboot. The installer now calls `systemctl restart`, which is correct for both the fresh-install and upgrade paths.
- **Helm: CertOps encryption keys were missing from the chart entirely** — `CERTOPS_SIGNING_ENCRYPTION_KEY` and `CERTOPS_REGISTRATION_ENCRYPTION_KEY` were wired in Compose but had no values key, ConfigMap entry, or Secret entry in `deploy/helm`, while `config.certopsEnabled` defaults to `true`. A stock Helm install therefore exposed the CertOps API but failed closed on agent registration and signed job dispatch (`CERTOPS_{SIGNING,REGISTRATION}_ENCRYPTION_KEY_MISSING`). Both are now first-class values (`config.certopsSigningEncryptionKey`, `config.certopsRegistrationEncryptionKey`), auto-generated as 64-hex-char keys when unset and read back from the existing Secret on `helm upgrade` like the other generated secrets.
- **Helm: the CertOps CronJob got zero egress under `networkPolicy.enabled=true`** — the worker NetworkPolicy's component allow-list omitted `certops`, so the default-deny policy matched the CertOps job pods and blocked all traffic including Postgres. This silently disabled every CertOps maintenance sweep (lease reaper, stale-agent sweep, renewal scheduler) in any install with NetworkPolicy on, including the `values-full-test.yaml` example.
- **Agent: `dnsPropagation.verificationMode` / `quorumCount` were silently ignored** — the startup config loader carried a duplicated copy of the propagation validation rules that returned only four of the six fields, and because `src/dns/hook.js` re-normalizes that object on every invocation, an operator-configured `quorum` policy degraded back to `all` with no error. The loader now delegates to the single validator in `src/dns/propagate.js`, so the two documented fields take effect and an invalid `quorum` config fails loud at startup.
- **Helm: post-install notes and the full-test example omitted the CertOps CronJob** — `NOTES.txt` listed only five of the six worker CronJobs, and `examples/values-full-test.yaml` (documented as "all workers enabled") left `worker.cronjobs.certops` to its inherited default.
