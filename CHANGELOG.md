...
- **Windows agent execution surface, groundwork (CertOps, targets 0.12.0).** Signed job dispatch, a service-aware Windows host with ACL-protected agent credentials, diagnostic-agent isolation, node-free reference clients, and the schema and storage groundwork for IIS and Windows certificate-store targets, delivered as a stack of PRs. **This release does not perform Windows deployments:** there is no IIS certificate-binding or machine-store execution path yet, and CNG-native (non-exportable) key custody is not implemented. Accordingly, `os-store-managed` key custody is reported as not agent-deployable, and the three capabilities that depend on a real host execution path (`windows-cert-store-v1`, `iis-binding-v1`, `trust-anchor-deploy-v1`) are never advertised. A later release adds the executors and qualifies those capabilities in the same change, so a job can never be dispatched to a path that cannot run it. The PRs:
