...
- **Workspace token transfer left endpoint monitors and managed certificates in the source workspace.** `POST /api/v1/workspaces/:id/transfer-tokens` now moves those rows, plus remapped contact-group assignments, in the same transaction. A fingerprint or job idempotency collision in the destination returns 409 `TRANSFER_CONFLICT` and leaves the source unchanged. (#265)
