...
#     Control Manager's control protocol (StartServiceCtrlDispatcher).
#     Registering it via sc.exe/New-Service is the native-tooling
#     mechanism this task requires, but whether the SCM considers the
#     resulting service healthy long-term needs verification on a real
#     Windows host; this script's health check only observes
#     Get-Service's reported status after start, which is what is
...
if (-not $healthy -and $script:IsUpgrade -and (Test-Path $script:AppPrevious)) {
    ...
    $rollbackBinPath = "$quotedNode $quotedEntry"
