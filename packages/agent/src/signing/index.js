...
// allocation), and 65536/49152 satisfy floor(65536/4)*3 = 49152 exactly, so
// the two bounds describe the same physical ceiling in two units.
const V2_MAX_ENCODED_PAYLOAD_CHARS = 65536;
 ...
 * UTF-8 decode -> JSON.parse. No canonicalization step exists in this path
 * by design: the signed bytes ARE the wire bytes, so there is nothing to
 * re-derive. A verifier in any language needs only "base64-decode, then
 * verify the raw bytes" to interoperate with this contract.
 *
