...
#   -WorkspaceId UUID            Workspace id, if required.
#   -BootstrapTokenFile PATH     File holding the bootstrap token.
...
$script:RegisterPath = "/api/v1/certops/agent/register"
...
function Read-SecretFile {
    ...
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
        Exit-Usage "credential/bootstrap-token file not found: $Path"
    }
...
function Confirm-V2Envelope {
    ...
    if (-not [string]::IsNullOrEmpty($ExpectedAgentId) -and -not [string]::IsNullOrEmpty($agentId) -and $agentId -ne $ExpectedAgentId) {
...
function Invoke-StepRegister {
    ...
    if ([string]::IsNullOrEmpty($BootstrapTokenFile)) {
        Exit-Usage "-Step register requires -BootstrapTokenFile"
    }
    $bootstrapToken = Read-SecretFile $BootstrapTokenFile
    $body = '{"agentVersion":"' + (ConvertTo-JsonEscaped $AgentVersion) + '","declaredTargetSelectors":[],"declaredCommandProfileNames":[],"declaredCapabilities":[]}'
    $envelope = New-OutboundEnvelope -MessageType "register" -BodyJson $body -AgentIdValue ""
    $result = Invoke-BoundedHttpPostJson -Url ($ServerUrl + $script:RegisterPath) -Token $bootstrapToken -JsonBody $envelope -MaxResponseBytes $script:MaxClaimResponseBytes
    $bootstrapToken = $null

    if ($result.StatusCode -ne 200 -and $result.StatusCode -ne 201) {
        Exit-Network "registration failed with HTTP $($result.StatusCode)"
    }
    $responseObj = ConvertFrom-SingleJsonValue $result.BodyBytes
    if ($null -eq $responseObj) {
        Exit-Pregate "registration response is not valid JSON"
    }
    ...
    if ([string]::IsNullOrEmpty($responseAgentId) -or $responseAgentId -notmatch $script:IdPattern) {
        Exit-Pregate "registration response is missing a valid agentId"
    }
    ...
    Write-Info "registered as agentId=$responseAgentId"
...
function Invoke-StepHeartbeat {
    ...
    $body = '{"agentVersion":"' + (ConvertTo-JsonEscaped $AgentVersion) + '"}'
...
function Invoke-StepClaim {
    ...
    $credential = Resolve-AgentCredential $CredentialFile
    $body = '{"maxJobs":1,"supportedActions":["noop"]}'
...
function Invoke-StepResult {
    ...
    if ([string]::IsNullOrEmpty($script:VerifiedJobId)) {
        Exit-Usage "-Step result requires a job already verified in this run (use -Step all, or -Step claim then -Step result in the same process)"
    }
    $credential = Resolve-AgentCredential $CredentialFile
...
function Invoke-StepAll {
    Assert-ServerUrlRequired
    if ([string]::IsNullOrEmpty($BootstrapTokenFile)) {
        Exit-Usage "-Step all requires -BootstrapTokenFile"
    }
    if ([string]::IsNullOrEmpty($Pubkey)) {
    ...
    $bootstrapToken = Read-SecretFile $BootstrapTokenFile
    $registerBody = '{"agentVersion":"' + (ConvertTo-JsonEscaped $AgentVersion) + '","declaredTargetSelectors":[],"declaredCommandProfileNames":[],"declaredCapabilities":[]}'
    $registerEnvelope = New-OutboundEnvelope -MessageType "register" -BodyJson $registerBody -AgentIdValue ""
    $registerResult = Invoke-BoundedHttpPostJson -Url ($ServerUrl + $script:RegisterPath) -Token $bootstrapToken -JsonBody $registerEnvelope -MaxResponseBytes $script:MaxClaimResponseBytes
    $bootstrapToken = $null
    if ($registerResult.StatusCode -ne 200 -and $registerResult.StatusCode -ne 201) {
        Exit-Network "registration failed with HTTP $($registerResult.StatusCode)"
    }
    ...
    if ([string]::IsNullOrEmpty($credential)) {
        Exit-Pregate "registration response is missing a credential"
    }
    Write-Info "registered as agentId=$resolvedAgentId"

    $heartbeatBody = '{"agentVersion":"' + (ConvertTo-JsonEscaped $AgentVersion) + '"}'
    ...
    $claimBody = '{"maxJobs":1,"supportedActions":["noop"]}'
