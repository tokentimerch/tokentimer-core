...
function App() {
  ...
  const fetchTokensForCategory = useCallback(
    async (categoryValue, opts = { reset: false }) => {
      ...
      try {
        ...
        if (opts.reset) setIsRefreshing(true);
        const nextOffset = opts.reset
          ? 0
        ...
        // If workspace changed while request was in-flight, skip applying results
        const currentWs =
          new URLSearchParams(window.location.search).get('workspace') || null;
        if (currentWs === selectedWorkspaceId) {
          setTokens(prev => {
  ...
  const fetchGlobalFacets = useCallback(
    async (opts = {}) => {
      try {
        ...
        // Verify state is still current
        const currentParams = new URLSearchParams(location.search);
        if (currentParams.get('workspace') === selectedWorkspaceId) {
          setGlobalFacets({
...
function DashboardView({
  ...
  const handleAddDomain = async () => {
    ...
    try {
      await apiClient.post(`/api/v1/workspaces/${ctxWorkspaceId}/domains`, {
        url: domainUrl.trim(),
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      _setTokens(prev => prev.filter(t => !deletedIds.includes(String(t.id))));
  ...
  return (
    <Box>
      ...
      {/* Endpoint (SSL) Monitor Modal */}
      <Modal isOpen={isDomainModalOpen} onClose={onDomainModalClose} size='xl'>
        <ModalOverlay />
        <ModalContent maxW='1100px'>
          <ModalHeader>Endpoint (SSL) Monitor</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
                      ...
                      {domains.map(d => (
                        ...
                        >
                          <HStack justify='space-between' align='start' mb={2}>
                            ...
                            <HStack spacing={1}>
                              <IconButton
                                ...
                                onClick={() => handleCheckDomain(d.id)}
                          ...
                          <HStack spacing={2} flexWrap='wrap' mb={2}>
                            ...
                            <Badge variant='outline'>{d.check_interval}</Badge>
                  ...
                  >
                    <Table size='sm' variant='simple'>
                      <Thead>
                        <Tr>
                          <Th>Endpoint</Th>
                          <Th whiteSpace='nowrap'>SSL</Th>
                          <Th whiteSpace='nowrap'>Health</Th>
                          ...
                          <Th textAlign='right' whiteSpace='nowrap'></Th>
                        </Tr>
                      </Thead>
                      <Tbody>
                        {domains.map(d => (
          ...
          <ModalFooter>
            <Button onClick={onDomainModalClose}>Close</Button>
          </ModalFooter>
        </ModalContent>
      </Modal>
    </Box>
