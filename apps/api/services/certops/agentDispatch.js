...
async function recordHeartbeat({
  ...
  return await withTransaction(dbPool, async (client) => {
    ...
    // ADR-0012 decision 17 requires an audit event on any actual write to
    // declared_capabilities (never on the absent-preserves no-op path), and
    // that event's payload names both the old and new sets. The pre-write
    // read only runs when a write is actually about to happen (declared
    // list is present on the wire), so an ordinary heartbeat that omits
    // capabilities costs no extra round trip.
    ...
    // only other place declared_capabilities is written). Fired only on an
    // actual write to the column - never on the absent-preserves no-op path
    // - so a downgrade (or any capability change) is reconstructable after
    // the fact instead of leaving no trail.
    if (declaredCapabilities !== null) {
      await auditWriter({
        client,
        actorUserId: null,
