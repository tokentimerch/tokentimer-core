...
describe("CertOps certificate renewal-state derivation", () => {
  ...
  it("treats os-store-managed custody as renewable now that the Windows executor exists", () => {
    // Historically stayed not-eligible pending the real Windows
    // store/site/binding execution path; that executor now exists
    // (packages/agent/src/index.js's executeWindowsIisRenewJob), so
    // os-store-managed is auto-eligible like other agent-managed modes.
    const renewal = deriveCertificateRenewalState(
