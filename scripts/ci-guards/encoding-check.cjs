...
// Guard: catches UTF-16/BOM encoding mistakes at commit time. This
// project has hit a recurring, real bug where a tool-generated write
// produces UTF-16LE output (no BOM, alternating 0x00 bytes) instead of
// UTF-8, which then fails downstream JSON parsing and other text
// tooling silently until something chokes on it far from the write
// site. This guard scans every tracked text file for that exact byte
// pattern, plus a leading UTF-8 BOM (also disallowed), so the failure
// is caught at commit time instead of at parse time.
