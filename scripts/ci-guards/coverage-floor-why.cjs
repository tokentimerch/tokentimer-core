...
// Guard: closes the "coverage floors need mandatory why: rationale"
// finding from the M5c CI-guard backlog (tokentimer-canvas plan, "Quality
// and CI backlog" item #8). scripts/coverage-floors.cjs is the single
// source of truth for per-package coverage floors (consumed by
// scripts/check-coverage-threshold.js via --config/--package); this guard
// asserts every numeric threshold value in that file has a `// why:`
