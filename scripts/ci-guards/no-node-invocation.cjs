...
// Guard: verifies neither shipped reference client invokes Node. The
// two Node-free reference clients,
...
// structural no-op, not a proof of anything. If neither file exists yet
// on a given branch, this guard only proves "the guard itself runs
// without error", not "the clients never invoke Node" - there are no
// clients yet to check. It exits 0 vacuously in that case rather than
// failing, because failing for files that were never supposed to exist
// yet would be noise, not signal. Once the files land, this stops being
// vacuous automatically (same file paths, no guard change needed).
