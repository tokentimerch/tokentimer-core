...
// Guard: closes "the transactional/no-destructive-DDL migration check"
// from the M5c CI-guard backlog (tokentimer-canvas plan, "Quality and
// CI backlog" item #1, practice 7). Core shipped v0.11.3 to fix a
// migration-38 dedup collision, so a guard here has demonstrated value:
// this project's migrations are numbered SQL strings inside one array
// in apps/api/migrations/migrate.js (not separate .sql files), run in
