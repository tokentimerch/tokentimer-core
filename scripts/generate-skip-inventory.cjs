...
// Quality/CI backlog item 2: scans the test suite for every skipped test and
// requires each one to carry a machine-readable reason-tag classification,
// so a skip can never silently outlive the reason it was added for. A skip
// must be tagged with a `// skip-reason: <tag>` comment immediately above
// the skip call (tolerating a short run of explanatory comment lines above
// the tag itself), where `<tag>` is exactly one of:
//
